...
set a `RELEASE_PLEASE_TOKEN` secret (a PAT with `contents: write`) so the
release creation can trigger other workflows.
...
The version file (`src/atelier/_version.py`) is generated at build or install
time. Editable installs will not update the version until you rebuild or
reinstall.
