# Changelog

## [0.9.0](https://github.com/shaug/atelier/compare/v0.8.3...v0.9.0) (2026-07-27)


### Features

* add store-native notes mutation ([5af6c22](https://github.com/shaug/atelier/commit/5af6c22046d42a61371c27ecab03fadcae7c3846))
* **beads:** add process-backed client ([9d63b60](https://github.com/shaug/atelier/commit/9d63b6094e628ecbba5d05ad2edd180b5bdc500d))
* **beads:** add sync wrappers and test doubles ([e08f92d](https://github.com/shaug/atelier/commit/e08f92df91c8fc175fc1e8e9782adfe1a241cbf1)), closes [#580](https://github.com/shaug/atelier/issues/580)
* **messages:** define work-threaded coordination contract ([bf2ebe5](https://github.com/shaug/atelier/commit/bf2ebe54add6374c1086dff8a376562d8c476744))
* **messages:** demote agent inbox routing to compatibility ([3f98151](https://github.com/shaug/atelier/commit/3f981519211bcaf38e32b86581cf1ca044f96bed)), closes [#594](https://github.com/shaug/atelier/issues/594)
* migrate planner startup reads to AtelierStore ([e949d54](https://github.com/shaug/atelier/commit/e949d54a8d153df075aca46ca94f2d23a91c090c))
* **planner:** enforce richer bead authoring contract ([#559](https://github.com/shaug/atelier/issues/559)) ([f209363](https://github.com/shaug/atelier/commit/f2093637c023926edb3ab9b092ca8b45925e48c0)), closes [#556](https://github.com/shaug/atelier/issues/556)
* **planner:** migrate mailbox skills onto AtelierStore ([8954c84](https://github.com/shaug/atelier/commit/8954c84e0758f993ff4bfb9a1a077c76e51f10b4)), closes [#652](https://github.com/shaug/atelier/issues/652)
* **planner:** require full promotion previews ([#562](https://github.com/shaug/atelier/issues/562)) ([f643765](https://github.com/shaug/atelier/commit/f6437658393cc25aaca1766f5c4622e25a5093f5)), closes [#557](https://github.com/shaug/atelier/issues/557)
* **runtime:** add bounded planner and worker profiles ([8c5c4e6](https://github.com/shaug/atelier/commit/8c5c4e66a6839a64a984b53cc16ef79ee4b5f83b))
* **store:** add Beads-backed discovery reads ([94ef9ef](https://github.com/shaug/atelier/commit/94ef9efb08acfdb481330a6a5827a5f7f154e08c))
* **store:** add lifecycle mutation adapter ([3954fbe](https://github.com/shaug/atelier/commit/3954fbefa10c749d14454548ee82febad026eee0))
* **store:** migrate external ticket persistence ([641775f](https://github.com/shaug/atelier/commit/641775fe783c1ac643c29c3c670da32ac5e87225))
* **store:** migrate planner authoring mutations ([bdf8da3](https://github.com/shaug/atelier/commit/bdf8da399578582228de63abb7b4dbbf1c3a7ce9)), closes [#653](https://github.com/shaug/atelier/issues/653)
* **store:** migrate planner mutation skills to AtelierStore ([feb3447](https://github.com/shaug/atelier/commit/feb344766b04e67acc02ba23e119128e8e8d3a26)), closes [#653](https://github.com/shaug/atelier/issues/653)
* **testing:** define in-memory beads dispatcher harness ([7fd6ea6](https://github.com/shaug/atelier/commit/7fd6ea673bb9b634ff8f50a7c1c567489b0d6285)), closes [#467](https://github.com/shaug/atelier/issues/467)
* **worker:** add north-star self-review contract ([#561](https://github.com/shaug/atelier/issues/561)) ([c9b616c](https://github.com/shaug/atelier/commit/c9b616ccbffee81cbc5b50049ad5602dc62168cf))
* **worker:** capture restart runtime contract ([#572](https://github.com/shaug/atelier/issues/572)) ([0616125](https://github.com/shaug/atelier/commit/0616125374ec548468c8262ad6f2e4e964c47dcb)), closes [#569](https://github.com/shaug/atelier/issues/569)
* **worker:** define selected-scope worktree fast-path validation ([632afb7](https://github.com/shaug/atelier/commit/632afb79fce9e62a747b44e0943a966493cdbd3c))
* **worker:** process work-threaded blocking messages ([3795ea4](https://github.com/shaug/atelier/commit/3795ea407e0fb3bb246a8a49c22e365c93fb81e7)), closes [#593](https://github.com/shaug/atelier/issues/593)
* **worker:** restart at idle boundary after runtime updates ([#595](https://github.com/shaug/atelier/issues/595)) ([faaa738](https://github.com/shaug/atelier/commit/faaa738288b29ffe64b1298c77d9179f8654fdb3)), closes [#570](https://github.com/shaug/atelier/issues/570)
* **worker:** validate selected worktree scope before repair ([20cba3d](https://github.com/shaug/atelier/commit/20cba3ddeaa7be1e8406efe3dc53696b09520d88)), closes [#691](https://github.com/shaug/atelier/issues/691)


### Bug Fixes

* align plan-promote preview with planner contract ([9483896](https://github.com/shaug/atelier/commit/9483896bc80896ac0fd40e636e8bf5ab6e3a7768))
* **beads:** accept store-shaped dependency references ([80f516e](https://github.com/shaug/atelier/commit/80f516e87edcbd24f70fe7b39f28948d43be8c86))
* **beads:** add overflow repair command ([01033c6](https://github.com/shaug/atelier/commit/01033c67f1d526df81ec327ebeb85f62ff9bc3de))
* **beads:** centralize overflow repair client capability ([11354f1](https://github.com/shaug/atelier/commit/11354f1637c45bc9bfbbf7cadebaced85febdc07)), closes [#714](https://github.com/shaug/atelier/issues/714)
* **beads:** commit overflow repair writes via dolt server ([ce33974](https://github.com/shaug/atelier/commit/ce339747487263ac80b6bdb70ee021c6d801c080))
* **beads:** durably verify Dolt overflow repairs ([bda95b7](https://github.com/shaug/atelier/commit/bda95b769a297f29099731c11876f33df39c4b4d)), closes [#751](https://github.com/shaug/atelier/issues/751)
* **beads:** fail closed on oversized snapshot recovery ([6c10873](https://github.com/shaug/atelier/commit/6c108737dc3c4fcfdb2c043df999498d25c29a67))
* **beads:** finalize overflow repair verification ([25536f4](https://github.com/shaug/atelier/commit/25536f41caf1f68a8f8d169d06773dbb877cdb41))
* **beads:** gate json capability checks during inspection ([07c09db](https://github.com/shaug/atelier/commit/07c09db0dc3a976cadfd08af36c7f45a6bc5f5ba)), closes [#579](https://github.com/shaug/atelier/issues/579)
* **beads:** keep overflow matcher private ([18652fc](https://github.com/shaug/atelier/commit/18652fc05cdde77dd41f9c3dc93901c2e71753e2))
* **beads:** narrow dependency payload normalization ([c66e79f](https://github.com/shaug/atelier/commit/c66e79fbdf1d9cf5207ff835beea9eef73b805da))
* **beads:** persist dolt overflow repair SQL ([3cbcef0](https://github.com/shaug/atelier/commit/3cbcef03bf21b168e1a8583fae8c41a289cbfa4b))
* **beads:** recover helper-session restart failures ([ca781cc](https://github.com/shaug/atelier/commit/ca781ccb5703a6ba7aa5cee5207276d0125721cb))
* **beads:** repair overflowed issue history ([8b5d0a8](https://github.com/shaug/atelier/commit/8b5d0a8e0d1b7476fac8707a9c80d717b6bffa4a))
* **beads:** require helper-session proof for overflow repair ([0d0369f](https://github.com/shaug/atelier/commit/0d0369f94d1bda865e719da8e0a721db4acbeb30))
* **beads:** retry concurrent description metadata writes ([1618b79](https://github.com/shaug/atelier/commit/1618b79ba3600b55b3069c0349e14032263834d8)), closes [#713](https://github.com/shaug/atelier/issues/713)
* **beads:** use direct dolt sql for durable overflow repair ([b55cd10](https://github.com/shaug/atelier/commit/b55cd104b1b6e2f81267dcf5174f5dd537242fd7))
* **beads:** verify Dolt overflow compaction persistence ([b16da49](https://github.com/shaug/atelier/commit/b16da49e632461f51fa673c692d26a68d011b1f3))
* **beads:** verify Dolt overflow compaction persistence ([331ad5c](https://github.com/shaug/atelier/commit/331ad5c5970f834f5a499d44f803d9794385f61c))
* **beads:** verify sqlite overflow repair readback ([5d3b1e9](https://github.com/shaug/atelier/commit/5d3b1e993fa930a01326958549a2fb95110ccb81))
* **cli:** restore declared Click compatibility ([180f10e](https://github.com/shaug/atelier/commit/180f10ee15479fadce859662b2d4b3cd628777f0))
* **dev:** pin local quality gates to python 3.11 ([0af5f5c](https://github.com/shaug/atelier/commit/0af5f5c2877cc92632a08033ab7e806de122be6d))
* ensure local-create fast path restores epic worktree ([e171043](https://github.com/shaug/atelier/commit/e17104395c2cda63e6e10438835fbc78f5f911bf))
* **gc:** degrade integrated sha conflicts to anomalies ([4ed1198](https://github.com/shaug/atelier/commit/4ed1198b6d69de3e3b929a9943c3a122ae6e0948))
* **gc:** normalize placeholder null bead ids ([#632](https://github.com/shaug/atelier/issues/632)) ([086d1c4](https://github.com/shaug/atelier/commit/086d1c497313fc7aa3c09564f9adea7bac20b72b))
* **gc:** skip closed stale agent beads ([14f1ca4](https://github.com/shaug/atelier/commit/14f1ca4f8f1dca2868466b22a61edf043da73e48))
* **guardrails:** accept underscored re_split trigger notes ([327cc5e](https://github.com/shaug/atelier/commit/327cc5e032d5edc2a3420787a640c0784ee11cc8)), closes [#656](https://github.com/shaug/atelier/issues/656)
* handle legacy Beads tombstones at store boundary ([090a753](https://github.com/shaug/atelier/commit/090a7536dbfb6ac9bb6b8c209382f2c98f533251))
* harden overflow recovery for store-backed mutations ([e531328](https://github.com/shaug/atelier/commit/e531328b20f7f51abfb9dcc9c1168f896e775003))
* **install:** recover invalid project venv ([83411ac](https://github.com/shaug/atelier/commit/83411ac435d9f76dc5ea6b173332e4eaad145931))
* **install:** repair stale atelier launchers ([1788357](https://github.com/shaug/atelier/commit/17883578ab8480ed601487425ce45e116a53f984))
* **mail-inbox:** support explicit unread mode ([c27eb08](https://github.com/shaug/atelier/commit/c27eb08e9cdc00b319220083efef411e5f6b6581))
* **messages:** infer epic thread kind for top-level work ids ([c6420ad](https://github.com/shaug/atelier/commit/c6420adef44e5cb00c5477d951fee26c403dd8fa))
* **plan-promote-epic:** read canonical beads notes ([7be40bd](https://github.com/shaug/atelier/commit/7be40bd12c06089ebb451f29b350665f12e86f2a))
* **planner-startup:** harden projected skill runtime ([d5b7c23](https://github.com/shaug/atelier/commit/d5b7c2312e38f15a0c2059126c075082054e2c7a)), closes [#605](https://github.com/shaug/atelier/issues/605)
* **planner:** accept converged lifecycle promotion retries ([10f3bf5](https://github.com/shaug/atelier/commit/10f3bf58f3f6aca1a70e0630a2af5b2e4a4ede63))
* **planner:** accept structured guardrail authoring fields ([#637](https://github.com/shaug/atelier/issues/637)) ([844d54f](https://github.com/shaug/atelier/commit/844d54f092316662b5a691d16177dae19d3bbc2e)), closes [#634](https://github.com/shaug/atelier/issues/634)
* **planner:** clarify issue owner vs assignee ([ff10a06](https://github.com/shaug/atelier/commit/ff10a06064e2640427baedf4dd906ea90aa2a51c))
* **planner:** filter startup queued message output ([416428a](https://github.com/shaug/atelier/commit/416428a7d2b9044734e28fc29da46e1a537a63f1))
* **planner:** harden ownership helper runtime ([261353c](https://github.com/shaug/atelier/commit/261353c00d886129cd5cb5b016d3582bf7e7c805))
* **planner:** keep startup compatibility reads adapter-local ([d835a06](https://github.com/shaug/atelier/commit/d835a06732523699661125352125b50735dd6e6c))
* **planner:** move startup message compatibility reads into store ([5e116d7](https://github.com/shaug/atelier/commit/5e116d7d505a37e530eb82fb70664dec373badf5)), closes [#651](https://github.com/shaug/atelier/issues/651)
* **planner:** preserve inbox startup routing after rebase ([f15f369](https://github.com/shaug/atelier/commit/f15f369fd824a0c63aa231bd63524d8c7ec76002))
* preserve guardrails script default cwd behavior ([7697367](https://github.com/shaug/atelier/commit/7697367a29d1e602aa47641f5793ad2a304033c9))
* preserve legacy review-state aliases in store updates ([5153e8f](https://github.com/shaug/atelier/commit/5153e8f9795b03c2b737d635c6579f6612f09e5f))
* preserve overflow cause in review verification ([8666736](https://github.com/shaug/atelier/commit/86667362171f5129695e8e2b4ddc218ca9731d45))
* preserve stale manifest pythonpath during self-heal ([c55bf31](https://github.com/shaug/atelier/commit/c55bf310c825be6e3ff1cb5a13b81399a420a33b))
* **publish:** close exported GitHub issues from PR bodies ([#609](https://github.com/shaug/atelier/issues/609)) ([1afffc8](https://github.com/shaug/atelier/commit/1afffc89339773ffa78885354813b3009eb6bbd5)), closes [#607](https://github.com/shaug/atelier/issues/607)
* repair overflow verification readback ([2690910](https://github.com/shaug/atelier/commit/2690910a0e74cbdc05e6e4f3a84b2b7d7eebb270))
* repair planner helper runtime preflight for broken tool installs ([11bb37e](https://github.com/shaug/atelier/commit/11bb37e7aad49e0fba0c2d80edc17680235d222d))
* **runtime:** author bounded evidence from launch boundary ([d7801cc](https://github.com/shaug/atelier/commit/d7801cc06c51dd38ea33f80a3a67af86510a5d23))
* **runtime:** fail closed on evidence cleanup uncertainty ([a1a9d3d](https://github.com/shaug/atelier/commit/a1a9d3de778413d8432037f176bcdf8db95e7ae4))
* **runtime:** honor worker runtime overrides ([6a7d888](https://github.com/shaug/atelier/commit/6a7d8888bbb139f486df29f285c1eaa426361450))
* **runtime:** improve projected bootstrap diagnostics ([53b91c0](https://github.com/shaug/atelier/commit/53b91c0541ee670be99809ee5e01d8236a750cb1))
* **runtime:** isolate bounded worker evidence per iteration ([2161762](https://github.com/shaug/atelier/commit/216176223f69643c6ecc0887a53b176a9409ffdc))
* **runtime:** isolate projected skill dependency provenance ([1ffab95](https://github.com/shaug/atelier/commit/1ffab9560ad5990d7812102b4a2d9bf026d11594))
* **runtime:** preserve projected tool runtime roots ([acfed9d](https://github.com/shaug/atelier/commit/acfed9d2abba59287efc871d85179b52888b84f4))
* **runtime:** preserve split projected tool roots ([b00343d](https://github.com/shaug/atelier/commit/b00343def126ea0285a7c68df75cb477c689dd0f))
* **runtime:** stabilize projected repo python detection ([89e420d](https://github.com/shaug/atelier/commit/89e420dbf69f81054e96af992b6556004cd12ad3))
* **runtime:** support bounded convergence across worker agents ([0e0ecb2](https://github.com/shaug/atelier/commit/0e0ecb247b97ea2e481c81c5053bc6029000cf66))
* **skills:** backfill projected runtime manifests ([b4e7949](https://github.com/shaug/atelier/commit/b4e79491f9012c9ecdfda9e4deaa213c0226c27e))
* **skills:** bootstrap stale manifests from support runtime ([7e22108](https://github.com/shaug/atelier/commit/7e22108fa628425fd76a7cfb8477f5c6e61e08ae))
* **skills:** honor bootstrap repo root for beads defaults ([8755f90](https://github.com/shaug/atelier/commit/8755f90ad4be500ac8a8c6917f93359a7cf540b0))
* **skills:** package shared projected bootstrap tree ([#642](https://github.com/shaug/atelier/issues/642)) ([4d7ac6e](https://github.com/shaug/atelier/commit/4d7ac6e2965d014704b1ac57379c5fe862a8252f)), closes [#640](https://github.com/shaug/atelier/issues/640)
* **skills:** preserve PR base on update ([bcd4ac9](https://github.com/shaug/atelier/commit/bcd4ac95511bebb2d121b0c864b406f7b5e5df9e)), closes [#599](https://github.com/shaug/atelier/issues/599)
* **skills:** preserve work-done bootstrap runtime ([77faf35](https://github.com/shaug/atelier/commit/77faf3503ede5f401967d828bbc6223ee7da6c40)), closes [#704](https://github.com/shaug/atelier/issues/704)
* **skills:** repair projected bootstrap runtime resolution ([0affc6f](https://github.com/shaug/atelier/commit/0affc6fe5187d989aa16a878040a2fbc70f946a6)), closes [#760](https://github.com/shaug/atelier/issues/760)
* **skills:** repair stale copied agent-home runtime lookup ([637a9eb](https://github.com/shaug/atelier/commit/637a9ebfc701ba7455cb708d2463812e3f7fe31b)), closes [#762](https://github.com/shaug/atelier/issues/762)
* **skills:** repair stale projected runtime rollouts ([ef58a35](https://github.com/shaug/atelier/commit/ef58a35a3a0fe286ab13506c8eeebf5988549d88))
* **skills:** repair stale runtime without support manifest ([fbaa6b0](https://github.com/shaug/atelier/commit/fbaa6b02d9cb4ce829cbd5cce80874a2371cf015))
* **skills:** self-heal projected helpers on stale manifests ([bf33220](https://github.com/shaug/atelier/commit/bf332208b9c228095ca3e3bc83183c91c4786d41))
* speed up planner startup epic discovery ([e0d515d](https://github.com/shaug/atelier/commit/e0d515d533e19c6d3a1c71e3aef59668bf316796))
* **store:** collapse the public store boundary ([e2b42be](https://github.com/shaug/atelier/commit/e2b42be4249b8a7b1440c71c212173f702104111))
* **store:** keep message recipient hints adapter-local ([3ef96eb](https://github.com/shaug/atelier/commit/3ef96eb93e0c7c1be388e1f09e7ff294010b7562))
* **store:** keep the public store boundary abstract ([39e9fb4](https://github.com/shaug/atelier/commit/39e9fb484bbda5f77b05933e0cfdcc6af3d8bf36))
* **store:** move repair history behind beads clients ([f8458e9](https://github.com/shaug/atelier/commit/f8458e9a19a1e1d3f0aca544744a2ca9b4920635))
* **store:** narrow history convergence after empty stdout ([4b644b3](https://github.com/shaug/atelier/commit/4b644b30be290f9f581aa6796aba0e3e7b811d7e))
* **store:** publish persistence contract parity ([41f5cef](https://github.com/shaug/atelier/commit/41f5cefe76b58aaaaff388dedd8fd0fa3383a1e0))
* **store:** recover empty update stdout convergence ([c6a43c9](https://github.com/shaug/atelier/commit/c6a43c961fe5fce6926ff03ae4ee0a5571d6fd8f)), closes [#729](https://github.com/shaug/atelier/issues/729)
* **store:** remove extra public store protocol ([3e8b568](https://github.com/shaug/atelier/commit/3e8b5689faa3b960f838b8d4e11aadfe4b92500f))
* **store:** route review metadata writes through atelier store ([100cf4e](https://github.com/shaug/atelier/commit/100cf4e04c838f467299dff2c97286d80cdb32d3))
* **store:** tolerate missing agent-id lookups ([8e00ee1](https://github.com/shaug/atelier/commit/8e00ee14e177fb577c65a5e6819b042c3f98dbfc))
* **store:** verify multiline note appends ([70c5d82](https://github.com/shaug/atelier/commit/70c5d827cf871d4773f3ec3e6f2c8d5293f5ca7f))
* **store:** verify review updates with history evidence ([0cf79cd](https://github.com/shaug/atelier/commit/0cf79cdf2473649404725756f0a1074c7518598f))
* **worker:** avoid stalled startup epic scans ([7474d60](https://github.com/shaug/atelier/commit/7474d601d479dfb22733ccc802ff6dcbf62a6f4c))
* **worker:** bypass agent scan for hook updates ([e92ab0b](https://github.com/shaug/atelier/commit/e92ab0b944a81d71d8b5507e35aaaca23d9346e1))
* **worker:** demote stale hooked epics at startup ([3171b9b](https://github.com/shaug/atelier/commit/3171b9bf7f320eaa928a51b3dbc10867fcd0d534))
* **worker:** enforce bead-first review-feedback ordering ([#567](https://github.com/shaug/atelier/issues/567)) ([41d7b41](https://github.com/shaug/atelier/commit/41d7b41a5ef466e7df553edd8dda81b051a85592))
* **worker:** enforce bounded convergence evidence contract ([946845f](https://github.com/shaug/atelier/commit/946845fbdddefcbaf5e69357b0c98088e23af9df))
* **worker:** gate worktree reconciliation fallback ([f19a4c6](https://github.com/shaug/atelier/commit/f19a4c670f73401d9c1d9f435ad52713d380e2cf)), closes [#692](https://github.com/shaug/atelier/issues/692)
* **worker:** ignore inbox messages on terminal threads ([dcde917](https://github.com/shaug/atelier/commit/dcde9178a750707e2a64782fab6e5278e407ce1d))
* **worker:** ignore terminal inbox threads on startup ([f1b71ab](https://github.com/shaug/atelier/commit/f1b71ab063ad239ae6f70c91805e165aec4a3061))
* **worker:** keep blocked lifecycle mutation atomic ([5a4da87](https://github.com/shaug/atelier/commit/5a4da87b7f6909498d955ce70820bbde1b3e9d45))
* **worker:** keep blocked retry audit note stable ([b2429dd](https://github.com/shaug/atelier/commit/b2429dd6b6dcac6128061de274eb61c8daada457))
* **worker:** keep fast-path validation selected-scope-only ([d76b4fd](https://github.com/shaug/atelier/commit/d76b4fd8ab4fa5b61ec40635e97d155769f67e92))
* **worker:** keep merged closed children terminal during epic close ([a541da1](https://github.com/shaug/atelier/commit/a541da1d0dee09dad6ee7bf7eee93c490541e605))
* **worker:** keep stale closed PR inbox threads visible ([dd068fc](https://github.com/shaug/atelier/commit/dd068fc30a2d07e77ccb4066aa66ff0b6bbf2fa2))
* **worker:** narrow merged finalize close fallback ([846c208](https://github.com/shaug/atelier/commit/846c208d295075214b7161af67eb5ae4d00aa098)), closes [#726](https://github.com/shaug/atelier/issues/726)
* **worker:** preserve agent bead lookup coverage ([de49ca9](https://github.com/shaug/atelier/commit/de49ca93cab469dac9bc547621a0fc138abf385b))
* **worker:** preserve changeset review metadata during finalize ([#638](https://github.com/shaug/atelier/issues/638)) ([387a0e1](https://github.com/shaug/atelier/commit/387a0e1932a6fe1a44dd416b05e2c5778f55eac6)), closes [#636](https://github.com/shaug/atelier/issues/636)
* **worker:** preserve done-to-closed fallback repair ([3fabec1](https://github.com/shaug/atelier/commit/3fabec1b3f020f242f529b1e19b686623323ea35))
* **worker:** repair merged finalize close convergence ([ce85ea7](https://github.com/shaug/atelier/commit/ce85ea7eadbf1535edd98ec2fc917b41b22504ae))
* **worker:** scope blocked convergence to current episode ([d2ac85f](https://github.com/shaug/atelier/commit/d2ac85f9c0cf5b8b513032d87f3085a3cef42650)), closes [#721](https://github.com/shaug/atelier/issues/721)
* **worker:** stabilize blocked transition verification ([fd7df40](https://github.com/shaug/atelier/commit/fd7df40d4d93e3020588c5ef3f1d47319782ba60))
* **worker:** tighten startup review-followup diagnostics ([9f82f4d](https://github.com/shaug/atelier/commit/9f82f4db64079e12e9d8a20a1b59c8056fd32688)), closes [#661](https://github.com/shaug/atelier/issues/661)
* **worker:** verify close transitions after empty beads output ([cb76610](https://github.com/shaug/atelier/commit/cb76610ce81f70f6a8b7be1d7c2915b44f6e4421)), closes [#724](https://github.com/shaug/atelier/issues/724)

## [0.8.3](https://github.com/shaug/atelier/compare/v0.8.2...v0.8.3) (2026-03-06)

### Bug Fixes

- **config:** enforce sequential strategy for PR mode
  ([d3c8acd](https://github.com/shaug/atelier/commit/d3c8acdbf8175675e5da5dec8bfb8036c9bfa727))
- **config:** remove residual pr_strategy argument handling
  ([06fad3a](https://github.com/shaug/atelier/commit/06fad3aa36cf3393fec71caa5f19853acd006666))
- **config:** stop tracking branch pr strategy
  ([53e254a](https://github.com/shaug/atelier/commit/53e254ab02b77a3e949203150066c58b88d2bddd))
- **doctor:** classify authoritative and stale duplicate paths
  ([3adfd81](https://github.com/shaug/atelier/commit/3adfd81c5e5675c0fa75658c11a931006e1f3834))
- **prefix-migration:** auto-heal split worktree path drift
  ([eda298c](https://github.com/shaug/atelier/commit/eda298cf2c5d145d5a304489121c9235143b5a05)),
  closes [#540](https://github.com/shaug/atelier/issues/540)
- **worker:** block no-parent PR creation when sibling PR is active
  ([1493fee](https://github.com/shaug/atelier/commit/1493fee20f0692754788300688f5701e7d04fc53))
- **worker:** gate PR creation across full epic subtree
  ([f938782](https://github.com/shaug/atelier/commit/f9387826ca511b43c1f5a3f7981a433ca7464546))
- **worker:** remove legacy PR strategy runtime branches
  ([9b38b1d](https://github.com/shaug/atelier/commit/9b38b1d1c15ddf320f8b87766481fe94d8551c3d))

## [0.8.2](https://github.com/shaug/atelier/compare/v0.8.1...v0.8.2) (2026-03-05)

### Bug Fixes

- **agent-home:** drop ambient ATELIER_AGENT_ID overrides
  ([3d18a7d](https://github.com/shaug/atelier/commit/3d18a7d6516efe6934c1737412d63a1ce00862c6))
- **beads-context:** remove ambient repo-hint env fallbacks
  ([ca53088](https://github.com/shaug/atelier/commit/ca53088892659916bd560a9f824a3f11d634924d)),
  closes [#524](https://github.com/shaug/atelier/issues/524)
- **config:** default beads prefix prompt to existing value
  ([1ca42f8](https://github.com/shaug/atelier/commit/1ca42f841e0867126bf1553351e6f4655fe51b05))
- **planner-startup:** use planner worktree env for repo hints
  ([63c8474](https://github.com/shaug/atelier/commit/63c84749812fec10d42618a3802d6db985f2354c))
- **runtime-env:** align ambient warning text with explicit routing
  ([7cc6538](https://github.com/shaug/atelier/commit/7cc6538ff8cf38c95088940f83bfd9fda7f0f121)),
  closes [#526](https://github.com/shaug/atelier/issues/526)
- **runtime:** suppress self-scoped agent env warnings
  ([68afa88](https://github.com/shaug/atelier/commit/68afa884a73cd2ea2652ad6c0d4613d409926ab7)),
  closes [#523](https://github.com/shaug/atelier/issues/523)
- **worker:** allow startup worktree-path metadata override
  ([284c657](https://github.com/shaug/atelier/commit/284c657cbd920da221a97a4d44469720c4e8889f)),
  closes [#534](https://github.com/shaug/atelier/issues/534)
- **worker:** guard epic release with explicit runtime identity
  ([3c4aac5](https://github.com/shaug/atelier/commit/3c4aac5533dc5c8903321b8d7f2853ff3be28fea))
- **worker:** handle finalize dependency-gate soft failures
  ([c4bf366](https://github.com/shaug/atelier/commit/c4bf3668dabd6a30422a59519c0e8a203bebab92)),
  closes [#528](https://github.com/shaug/atelier/issues/528)

## [0.8.1](https://github.com/shaug/atelier/compare/v0.8.0...v0.8.1) (2026-03-04)

### Bug Fixes

- **agent-teardown:** fail closed on owner-only mismatch payloads
  ([de412fb](https://github.com/shaug/atelier/commit/de412fbcfa0d0905188d5be8160e8c41755be373))
- **agent-teardown:** fail closed on owner-only ownership payloads
  ([43de4ce](https://github.com/shaug/atelier/commit/43de4ce4bbea8a3f0465fbfeba74bc084eed7a74))
- **agent-teardown:** fail closed on ownership metadata drift
  ([c51f369](https://github.com/shaug/atelier/commit/c51f369832239f202ff0f5ab7c5874e536194955))
- **agent-teardown:** require epic release confirmation before closing agent
  bead
  ([9a58469](https://github.com/shaug/atelier/commit/9a58469ca5c8648e8937cd9af34b010f0a0d1ac3))
- **agent:** fail closed teardown close gate without hook target
  ([0d46a2c](https://github.com/shaug/atelier/commit/0d46a2c813ecdc8d861af361b3c02affb30f8ac6))
- **agent:** fail closed teardown ownership on index drift
  ([ef21d45](https://github.com/shaug/atelier/commit/ef21d459e6dc25b91d93d8356f4859fc94e662fe))
- **agent:** re-verify epic release before closing agent bead
  ([ea87161](https://github.com/shaug/atelier/commit/ea871617f13adb90d67910130004c48f06c857f6))
- **beads:** prefer open agent bead over closed duplicates
  ([fdc80e0](https://github.com/shaug/atelier/commit/fdc80e0bcf17ff2458836aa46624b3036ed4dce5)),
  closes [#506](https://github.com/shaug/atelier/issues/506)
- **doctor:** backfill missing mapping lineage in migration repair
  ([577f671](https://github.com/shaug/atelier/commit/577f671781c8bdd0c679c06dd1de990af5b4e1d6)),
  closes [#508](https://github.com/shaug/atelier/issues/508)
- **doctor:** converge prefix drift normalization to canonical branch paths
  ([1281e18](https://github.com/shaug/atelier/commit/1281e18246a5e04a3dbcec6dd211e12ceba2d0bb)),
  closes [#509](https://github.com/shaug/atelier/issues/509)
- **gc:** scan all agent beads during stale cleanup
  ([85600b1](https://github.com/shaug/atelier/commit/85600b11f1f5b0a0ea8bb277e7bc573a3addb4a9))
- **git:** forward git_path in enlistment resolution
  ([bf23369](https://github.com/shaug/atelier/commit/bf2336966f93e7b07f041d40fad4712c69ce2c68))
- **planner-startup:** resolve project identity from linked worktrees
  ([17672fb](https://github.com/shaug/atelier/commit/17672fb572cdcf7b22cede161b67200a992fc0a7)),
  closes [#519](https://github.com/shaug/atelier/issues/519)
- **prefix-migration:** converge legacy namespace drift deterministically
  ([700a0a5](https://github.com/shaug/atelier/commit/700a0a5315e688752e41be741b056c6c30cd3fd6)),
  closes [#510](https://github.com/shaug/atelier/issues/510)
- **prefix-migration:** enforce scoped changeset ownership
  ([6469171](https://github.com/shaug/atelier/commit/6469171b0a709e0e98af7745b0ccd38eb7f6bc7c))
- **prefix-migration:** fail closed on ambiguous worktree candidates
  ([71e1eda](https://github.com/shaug/atelier/commit/71e1eda17c249f09e0343864d87c6dea72b1e4de))
- **prefix-migration:** prefer remote canonical branch source
  ([1dedbf8](https://github.com/shaug/atelier/commit/1dedbf83138aae8312c20c30f5ef35afc8e34500))
- **prefix-migration:** preflight branch checkout before worktree moves
  ([c4df26e](https://github.com/shaug/atelier/commit/c4df26e24773ba63000fd8d9fc703e478f51ee81))
- **prefix-migration:** restore metadata worktree source resolution
  ([7a1c9e3](https://github.com/shaug/atelier/commit/7a1c9e37097669d9b9bc5f185ccec07c13487359))
- **runtime:** gate agent-bead closure on verified hook cleanup
  ([327dc64](https://github.com/shaug/atelier/commit/327dc640987854bf18633279266e0caf3804c023))
- **runtime:** harden planner teardown bead targeting
  ([e079574](https://github.com/shaug/atelier/commit/e079574d8225e41ce36314dbefda31db9c30aba0)),
  closes [#506](https://github.com/shaug/atelier/issues/506)
- **runtime:** teardown agent beads and hooks on exit
  ([485695a](https://github.com/shaug/atelier/commit/485695a79eb3ce0120d22af7570afcb6c2510f1a)),
  closes [#506](https://github.com/shaug/atelier/issues/506)
- **worker-review:** guard leaked review-branch pruning
  ([f9f2e4e](https://github.com/shaug/atelier/commit/f9f2e4e714bd07de07388cb5cbbd03e42a7678d8))
- **worker:** pass concrete agent bead id through teardown
  ([cdf38d9](https://github.com/shaug/atelier/commit/cdf38d98563f1d0850205ad773a3f6bed019f818)),
  closes [#506](https://github.com/shaug/atelier/issues/506)
- **worker:** stop leaking temporary pr-\* review branches
  ([aa5a537](https://github.com/shaug/atelier/commit/aa5a537180d60c2f36ca15ef1edd7f1285fca0dc)),
  closes [#507](https://github.com/shaug/atelier/issues/507)
- **worktrees:** fail closed on mismatched epic direct mapping
  ([d0d08ff](https://github.com/shaug/atelier/commit/d0d08ff56e27ef71bf8eef733197203d78e36f35))
- **worktrees:** prefer live branch source and fail closed on invalid canonical
  path
  ([739cfc1](https://github.com/shaug/atelier/commit/739cfc1d6bda48fbc38a3c41bb598c235b3d0559))
- **worktrees:** validate direct mapping owner on epic lookup
  ([d8bdd9f](https://github.com/shaug/atelier/commit/d8bdd9fefe89347f01077faac7fd6c9ac26e5aa2))

## [0.8.0](https://github.com/shaug/atelier/compare/v0.7.0...v0.8.0) (2026-03-04)

### Overview

This release hardens planner/worker runtime behavior with clearer role-scoped
execution, stronger lifecycle gating, and faster startup triage. For operators
upgrading from 0.7.x, 0.8.0 also makes runtime requirements explicit: newer `bd`
compatibility expectations, normalized Dolt server/runtime defaults, and
project-scoped prefix/db identity checks that now fail closed instead of
silently continuing on drift.

Agent experience is materially improved in day-to-day runs: Codex/Claude output
contracts are unified, Claude print-mode behavior is explicitly supported, and
live stream output now surfaces concise progress/tool activity with lower noise.

#### Operator impact

- Startup/runtime migration handling is stricter and safer, with bounded
  recovery and explicit degraded-mode diagnostics when legacy stores cannot be
  auto-repaired.
- Prefix normalization is now enforced across runtime and maintenance workflows;
  `doctor --fix` and related diagnostics surface and repair migration/prefix
  drift instead of deferring it.
- Ownership and project-prefix contracts are validated as hard gates so
  mis-scoped Beads/Dolt state fails fast with actionable guidance.

### Features

- **agent:** add role-scoped planner/worker launch options
  ([ef65753](https://github.com/shaug/atelier/commit/ef65753b7856014cd7bd8e0e112b1cf2502f0ad1))
- **agent:** add yolo startup mode for plan and work
  ([99463ca](https://github.com/shaug/atelier/commit/99463ca39ebb10720ea3450e393bc01f3448dc3e))
- **beads:** auto-migrate recoverable startup stores
  ([f03fa17](https://github.com/shaug/atelier/commit/f03fa1777e92398d092eba07bed27395bb9052c7)),
  closes [#200](https://github.com/shaug/atelier/issues/200)
- **beads:** classify startup legacy recovery state
  ([4902646](https://github.com/shaug/atelier/commit/49026460b59f0928ff55c871b3c632aa4a787a10))
- **beads:** converge legacy Dolt ownership during init and plan
  ([5188747](https://github.com/shaug/atelier/commit/51887474d9d91ddf8e4d9367ce0af5bd30baccb5)),
  closes [#412](https://github.com/shaug/atelier/issues/412)
- **beads:** emit startup auto-upgrade diagnostics
  ([f029cd1](https://github.com/shaug/atelier/commit/f029cd1cdc73da4e125934dcc92d4711c4b32c29))
- **beads:** generalize project-scoped prefix handling
  ([7d4bb98](https://github.com/shaug/atelier/commit/7d4bb98a1b3cc7721809e675755572566d07e9ec))
- **beads:** normalize dolt server runtime defaults
  ([1f48eee](https://github.com/shaug/atelier/commit/1f48eee9040414ebbf57ff96707bbdf424934fe4)),
  closes [#224](https://github.com/shaug/atelier/issues/224)
- **beads:** reconcile runtime agent beads after startup migration
  ([71778a7](https://github.com/shaug/atelier/commit/71778a7dcf1a0c0823c7aeac059a47cfe95a40b9)),
  closes [#201](https://github.com/shaug/atelier/issues/201)
- **beads:** supervise Dolt server lifecycle with bounded recovery
  ([5803687](https://github.com/shaug/atelier/commit/5803687e7eb691f655ca36e399cecc62a1efd037)),
  closes [#225](https://github.com/shaug/atelier/issues/225)
- **cli:** add prefix-migration doctor command
  ([dd9b2f1](https://github.com/shaug/atelier/commit/dd9b2f1a831d9a85a0db4389ae468f37330633d8)),
  closes [#462](https://github.com/shaug/atelier/issues/462)
- **commands:** add explicit prefix normalization workflow
  ([d4218ce](https://github.com/shaug/atelier/commit/d4218cebaa69e442442131933b79e6139bd0ca6b))
- **doctor:** expand migration health taxonomy
  ([c48ba7f](https://github.com/shaug/atelier/commit/c48ba7fb5264c51c2fcd94ceb613899f6f58fa99)),
  closes [#481](https://github.com/shaug/atelier/issues/481)
- **doctor:** expand project health diagnostics
  ([dbf3ee6](https://github.com/shaug/atelier/commit/dbf3ee60c284ceafe49aace33c9a2271028ee304)),
  closes [#478](https://github.com/shaug/atelier/issues/478)
- **external-tickets:** mirror github parent-child issue hierarchy
  ([e0ad54c](https://github.com/shaug/atelier/commit/e0ad54cffdc28ff9effb43af8c561b6c05bf3dd2)),
  closes [#208](https://github.com/shaug/atelier/issues/208)
- **gc:** add at:ready and at:draft removal; remove vestigial labels from tests
  ([23f0d97](https://github.com/shaug/atelier/commit/23f0d979ae4e12ed62ff35973e61a860fa897cd6))
- **gc:** add cs:\* label removal; remove vestigial cs: from tests
  ([c744480](https://github.com/shaug/atelier/commit/c7444804bae45fccc2e2718b5a170a5859a62417))
- **lifecycle:** define graph-status lifecycle contract
  ([84a3aff](https://github.com/shaug/atelier/commit/84a3affd58ce2359cc1637b6dce6c502aee36039)),
  closes [#255](https://github.com/shaug/atelier/issues/255)
- **open:** make workspace resolution changeset-centric
  ([7c276a8](https://github.com/shaug/atelier/commit/7c276a876a6b03796a036dcae2e13e4baa9c932b)),
  closes [#446](https://github.com/shaug/atelier/issues/446)
- **planner-startup:** add canonical startup command plan
  ([530f6f5](https://github.com/shaug/atelier/commit/530f6f5926ac78ebf83e6ff28d932a9d37c084e1)),
  closes [#475](https://github.com/shaug/atelier/issues/475)
- **planner:** add structured startup triage renderer
  ([ad7add2](https://github.com/shaug/atelier/commit/ad7add292ed515fbc0aa08af5534c64425a5068c)),
  closes [#476](https://github.com/shaug/atelier/issues/476)
- **planner:** surface deferred changesets in startup overview
  ([55d112b](https://github.com/shaug/atelier/commit/55d112b9b5a72ba81fbe0ae6030e80c5f94dc467)),
  closes [#329](https://github.com/shaug/atelier/issues/329)
- **plan:** resume planner sessions by default
  ([cae954f](https://github.com/shaug/atelier/commit/cae954f7cdc0d8ad8f211e4ebfe9e514b1d85723))
- **scripts:** add --atelier-path to control worker executable
  ([2ac7f6a](https://github.com/shaug/atelier/commit/2ac7f6ad9f6d6516102523d38b3f08a11e9f93ad))
- **scripts:** add self-updating worker supervisor runner
  ([2914c64](https://github.com/shaug/atelier/commit/2914c6472251ad5e76968089032951f47a0ae6df))
- **scripts:** add supervisor --yes flag and make worker consent explicit
  ([e66424f](https://github.com/shaug/atelier/commit/e66424ff5a0ccfe95d791a34b4c2f093be848146))
- separate planner vs worker launch options and enable Claude worker print mode
  ([1c3d03e](https://github.com/shaug/atelier/commit/1c3d03e0da91fb83e314ec269d06620afc85d22f))
- **skills:** strengthen cross-cutting changeset guardrails
  ([f9f748d](https://github.com/shaug/atelier/commit/f9f748dbc19cd893606610b585759344dfad7da5)),
  closes [#352](https://github.com/shaug/atelier/issues/352)
- **startup:** auto-import legacy beads data on first operations
  ([6cc3153](https://github.com/shaug/atelier/commit/6cc3153a4eab118dc432e4884a74543d4e605fe0)),
  closes [#302](https://github.com/shaug/atelier/issues/302)
- **status:** add prefix-migration drift scanner
  ([7826bf3](https://github.com/shaug/atelier/commit/7826bf30f1f6cdcd89482c77b63dade9cd3951b0)),
  closes [#460](https://github.com/shaug/atelier/issues/460)
- **worker-session:** unify cross-agent output render contract
  ([9a7a785](https://github.com/shaug/atelier/commit/9a7a78504306dd03e9d92e62dfe2849c0d690ecd)),
  closes [#454](https://github.com/shaug/atelier/issues/454)
- **worker:** add Codex --json output, split output into agent modules
  ([e614141](https://github.com/shaug/atelier/commit/e614141db7d8dc259ffcc45b813fbe464a8eb02f))
- **worker:** add startup selector policy and faster review scans
  ([a0f6006](https://github.com/shaug/atelier/commit/a0f6006f1819622c7814c85f36f939a36530790e)),
  closes [#436](https://github.com/shaug/atelier/issues/436)
- **worker:** default to full codex reasoning with formatted live lines
  ([37e1b82](https://github.com/shaug/atelier/commit/37e1b82a1d0a7630335d83f9a76452afe44817cc))
- **worker:** require explicit at-ready gating for executable work
  ([8292802](https://github.com/shaug/atelier/commit/82928029c6e5148c59d275933ff847a1fe4c64d3))
- **worker:** short-circuit startup for terminal changesets
  ([482f451](https://github.com/shaug/atelier/commit/482f451abe9f7756b95d19ee913cb1b4e9153fde)),
  closes [#485](https://github.com/shaug/atelier/issues/485)
- **worker:** show concise codex tool activity in live stream
  ([20a045b](https://github.com/shaug/atelier/commit/20a045bae637953a694cce88edecfeca8c9cff27))
- **worker:** stream low-noise agent progress for codex and claude
  ([df45897](https://github.com/shaug/atelier/commit/df45897ad6fb0a1030859a26ddbfdf74405298de))
- **worker:** surface bounded codex reasoning in live output
  ([afe9c87](https://github.com/shaug/atelier/commit/afe9c879cb6dd50efa8a4ed8cac3805ec250fbbe))

### Bug Fixes

- add `--yes` to runtime
  ([c883592](https://github.com/shaug/atelier/commit/c8835929693fb0b4920979bf545a9b3133ea878d))
- **agent-home:** resolve Claude SessionStart hook path
  ([bee00f8](https://github.com/shaug/atelier/commit/bee00f8457c5966afb66aa9b2f76478761c2261e)),
  closes [#422](https://github.com/shaug/atelier/issues/422)
- **agent:** enable claude yolo permissions flag
  ([ed8b463](https://github.com/shaug/atelier/commit/ed8b46368f1d7b85930555a1c46935be6e4cd29d))
- **agent:** ensure Claude worker print stream-json launch compatibility
  ([59868a6](https://github.com/shaug/atelier/commit/59868a6c76ebded7d4817502a6cae012f6941672)),
  closes [#440](https://github.com/shaug/atelier/issues/440)
- **agents:** avoid swallowing short flags in option merge
  ([875823f](https://github.com/shaug/atelier/commit/875823f3a85e712055676579900374839e56fd8c))
- **agents:** inject atelier pythonpath for skill scripts
  ([cf70259](https://github.com/shaug/atelier/commit/cf70259f73335c77dc64474f4369d0faaa8f9325))
- **agents:** preserve last-wins for single-dash option values
  ([1ce8f15](https://github.com/shaug/atelier/commit/1ce8f158d8471a5d2b9bcb6ac4f5d2f2a39bbdc0))
- **auto-export:** guard external ticket body rendering
  ([a301e7c](https://github.com/shaug/atelier/commit/a301e7ce528980961bbab6f9b3bc378c8f2e82d9))
- **auto-export:** narrow output heading contamination guard
  ([3fc8393](https://github.com/shaug/atelier/commit/3fc83935d52e3ca6c8b4f56d883162115e40d723))
- **beads:** avoid duplicate runtime normalization during bd commands
  ([600f8ce](https://github.com/shaug/atelier/commit/600f8ce135d54365b375fdba33981afd87cc718a))
- **beads:** avoid false-positive pending collection detection
  ([823ae8a](https://github.com/shaug/atelier/commit/823ae8a6524d4267ef4548ced42ae2d56d90d21a)),
  closes [#359](https://github.com/shaug/atelier/issues/359)
- **beads:** avoid title-contains planner crash on dolt stores
  ([68300b0](https://github.com/shaug/atelier/commit/68300b02d18916c02c92b9d6902b3545b6291b11))
- **beads:** clarify runtime DB reconciliation fallback diagnostics
  ([424cb91](https://github.com/shaug/atelier/commit/424cb9185f81f8e788a50198fc65497e5e4e6485)),
  closes [#415](https://github.com/shaug/atelier/issues/415)
- **beads:** distinguish blocked runtime db reconciliation outcomes
  ([668f2c7](https://github.com/shaug/atelier/commit/668f2c71f44ffbcc9bc7d851c5df651dea23bf3f))
- **beads:** enforce minimum bd version
  ([08b07a7](https://github.com/shaug/atelier/commit/08b07a795940e63a0cb76f279893f8929147cc7a))
- **beads:** enforce project-scoped Dolt database ownership
  ([2f5782c](https://github.com/shaug/atelier/commit/2f5782c6db5d2fb3bb30f01b58f1e6497814271e)),
  closes [#421](https://github.com/shaug/atelier/issues/421)
- **beads:** fail closed before clearing stale unread labels
  ([ed9f56f](https://github.com/shaug/atelier/commit/ed9f56f660c8ff697184d8e852057591065336d9))
- **beads:** fail closed on ambiguous database candidate sets
  ([e922ce2](https://github.com/shaug/atelier/commit/e922ce2b1544b31240a930dfa0a76d98ea963e4b)),
  closes [#412](https://github.com/shaug/atelier/issues/412)
- **beads:** fail closed on ambiguous dolt database selection
  ([47bcc7f](https://github.com/shaug/atelier/commit/47bcc7f5de779e6b00c59a68755d3529710d5b8f))
- **beads:** fail closed on ambiguous startup runtime DB convergence
  ([bb7983b](https://github.com/shaug/atelier/commit/bb7983b6af480c2f9ec4e1b165abc9f67a38b6ad)),
  closes [#415](https://github.com/shaug/atelier/issues/415)
- **beads:** fail closed on claim and stale message release
  ([65236de](https://github.com/shaug/atelier/commit/65236decec6f898eae15113216f6923171c6b74f))
- **beads:** fail closed on cross-project prefix collisions
  ([b6507e4](https://github.com/shaug/atelier/commit/b6507e46b3b9974009efa87615439e6d866b11fd))
- **beads:** fail closed on project prefix/database mismatch
  ([21f29a4](https://github.com/shaug/atelier/commit/21f29a40c353b8fba09d28fd64c1a5abb47aad0f))
- **beads:** fail startup health when dolt store is missing
  ([9ccd84c](https://github.com/shaug/atelier/commit/9ccd84cd603932f9524c0bf79625bf32f8b3596c)),
  closes [#368](https://github.com/shaug/atelier/issues/368)
- **beads:** gate bd dolt commit on runtime need
  ([00710df](https://github.com/shaug/atelier/commit/00710dfcd0f96d161707493edf663394c73b9bb3)),
  closes [#359](https://github.com/shaug/atelier/issues/359)
- **beads:** harden concurrent claim and metadata updates
  ([d80e9c5](https://github.com/shaug/atelier/commit/d80e9c52eeb579e4a546b3364eb69e5a18b27cf2)),
  closes [#371](https://github.com/shaug/atelier/issues/371)
- **beads:** harden concurrent claim and metadata writes
  ([9171a4c](https://github.com/shaug/atelier/commit/9171a4c1c884d975d61f1992a79a495b48a9e6ca))
- **beads:** harden dolt preflight and init prefix rename flow
  ([6419d59](https://github.com/shaug/atelier/commit/6419d5969c2a7fc9aaa638744fd33c275f8c8bd3))
- **beads:** harden fallback bd invocation guardrails
  ([31ef5da](https://github.com/shaug/atelier/commit/31ef5dae3011e23d4ac87471e123d03183e13ab1)),
  closes [#473](https://github.com/shaug/atelier/issues/473)
- **beads:** harden issue write lock acquisition
  ([06b15a6](https://github.com/shaug/atelier/commit/06b15a6b1ee757917c2036c884b58e425567000e))
- **beads:** harden non-interactive prefix and epic compatibility scans
  ([a51baba](https://github.com/shaug/atelier/commit/a51baba979b97269aa39a420684d05fc96405201))
- **beads:** harden project-scoped dolt database ownership
  ([a7c793e](https://github.com/shaug/atelier/commit/a7c793e433c250cc939c969f017fc268a7207be5))
- **beads:** harden startup auto-migration preflight
  ([b17be21](https://github.com/shaug/atelier/commit/b17be21670abe95d9504e87cbcb5e22f4430504e))
- **beads:** honor configured dolt database in ambiguous ownership
  ([72816c6](https://github.com/shaug/atelier/commit/72816c6c45867d2fbea24f9e162aeb4419e3fb77))
- **beads:** keep dolt database naming prefix-only
  ([c3b4bc9](https://github.com/shaug/atelier/commit/c3b4bc90bed2bd5813cdc63640756d56823998ad))
- **beads:** keep lifecycle labels in fixed at namespace
  ([4b1f4e6](https://github.com/shaug/atelier/commit/4b1f4e6a291a3803c41aa2c41f3c2386dab0acd4))
- **beads:** make prime guidance capability-aware
  ([5ff7237](https://github.com/shaug/atelier/commit/5ff7237bdcc44f11a8be290527e21dcd5b5de589))
- **beads:** make prime guidance normalization capability-aware
  ([932646a](https://github.com/shaug/atelier/commit/932646ad7efc2c067d13f2d1a91535671d7e70d1))
- **beads:** narrow legacy label compatibility scope
  ([fea25cc](https://github.com/shaug/atelier/commit/fea25cc8d715e894d072bbd015811d20a9f4f234))
- **beads:** normalize runtime db reconciliation
  ([b2eb80b](https://github.com/shaug/atelier/commit/b2eb80b47ca17a28b8693c7b6b028bdcc7ce0c65))
- **beads:** pin bd database path for stable issue reads
  ([f5089f8](https://github.com/shaug/atelier/commit/f5089f81ba0f44fcdb475799ef02e61f94c2bd9a))
- **beads:** pin BEADS_DB to scoped beads root
  ([4a46929](https://github.com/shaug/atelier/commit/4a46929446b28579fa0500250d91b966595fac68))
- **beads:** preserve dolt database identity during startup convergence
  ([bfad789](https://github.com/shaug/atelier/commit/bfad789f84aee08b18aed9fc809b4f5aa4e73075))
- **beads:** preserve explicit override recovery
  ([917377b](https://github.com/shaug/atelier/commit/917377be4f4888dad58af011d6c6cbf726c12733))
- **beads:** preserve external ticket metadata on description edits
  ([f76c5eb](https://github.com/shaug/atelier/commit/f76c5ebae7b481974f132ebc29c2f8d69ffa8de6)),
  closes [#174](https://github.com/shaug/atelier/issues/174)
- **beads:** reconcile exported tickets on epic close paths
  ([3096ba2](https://github.com/shaug/atelier/commit/3096ba2018d1960061a7828d2205e1e33875da0d)),
  closes [#207](https://github.com/shaug/atelier/issues/207)
- **beads:** remove export rewrites from prime guidance
  ([311f338](https://github.com/shaug/atelier/commit/311f338ad341a64a5aa738ef4487fc605c62cc79)),
  closes [#232](https://github.com/shaug/atelier/issues/232)
- **beads:** remove runtime prime guidance rewrites
  ([d8d85f5](https://github.com/shaug/atelier/commit/d8d85f5425d7dccfea0bc72316a6a2f863273924)),
  closes [#232](https://github.com/shaug/atelier/issues/232)
- **beads:** replace deprecated sync guidance
  ([9d53fb0](https://github.com/shaug/atelier/commit/9d53fb08ccf9615016cee9d278f7b692c6db21d8))
- **beads:** route lock acquisition failures through die
  ([a06c6c4](https://github.com/shaug/atelier/commit/a06c6c4aaa213cf33a1c4d748627b2f036a01cb4))
- **beads:** sanitize configured runtime dolt database
  ([cb20420](https://github.com/shaug/atelier/commit/cb204203ed517927623b28c54fc99a989f35246a))
- **beads:** skip dolt preflight for startup prime
  ([5dd92aa](https://github.com/shaug/atelier/commit/5dd92aad9953fa4536da953a35c76628d47a540b))
- **beads:** strip unsupported bd export from prime addendum
  ([8499a77](https://github.com/shaug/atelier/commit/8499a7733665abac181f06447ab49a6927c81cfc))
- **beads:** strip unsupported bd export from prime addendum
  ([73dfe1a](https://github.com/shaug/atelier/commit/73dfe1ae91c2b31d9307309f50a36b6d967e7011))
- **beads:** surface degraded-mode guidance for dolt recovery failures
  ([0275186](https://github.com/shaug/atelier/commit/0275186648836837d506b7aa50b5e124328cee74)),
  closes [#226](https://github.com/shaug/atelier/issues/226)
- **beads:** tolerate missing queue thread lookup
  ([41a41ac](https://github.com/shaug/atelier/commit/41a41acbf2b2d0c508cf3727dde130293b4cdc6f))
- **cli:** show constrained choices in command help
  ([847aaa5](https://github.com/shaug/atelier/commit/847aaa535444323588b889166e25c910ddb71c6b))
- **codex:** disable raw tty input for non-streamed runs
  ([ecfac2d](https://github.com/shaug/atelier/commit/ecfac2d59e50a9151980a649913c32003926949f))
- **concurrency:** harden non-beads external writes
  ([e3d7aba](https://github.com/shaug/atelier/commit/e3d7abaa1b86781343451b5aee3cda04fb81dee8)),
  closes [#374](https://github.com/shaug/atelier/issues/374)
- **dependency-lineage:** fail closed when heritage parent lookup is missing
  ([d756c0d](https://github.com/shaug/atelier/commit/d756c0dad112aa04877b8ca07d00321995927b39)),
  closes [#407](https://github.com/shaug/atelier/issues/407)
- **dependency-lineage:** honor normalized issue heritage
  ([4099c37](https://github.com/shaug/atelier/commit/4099c37efa00dc41d27c2a0fffa6ba446ff2e6b8))
- **dependency-lineage:** scope base lineage to epic heritage
  ([21d9538](https://github.com/shaug/atelier/commit/21d95382434e52a54913bf992742eba71eccb0a7))
- **doctor:** gate --fix when active agent hooks exist
  ([f824392](https://github.com/shaug/atelier/commit/f824392704ef83f9a26a8dcfde0064be5d9b4afb))
- **doctor:** make doctor the sole prefix normalization entrypoint
  ([52762a5](https://github.com/shaug/atelier/commit/52762a536e6c51fea833aaf0819c1383ac5c7086))
- **doctor:** refine metadata readiness blocker semantics
  ([4481d28](https://github.com/shaug/atelier/commit/4481d28c7edeb05af59eedab4d69ca946e37994e))
- **external-tickets:** reconcile exported github issues on close
  ([51f6a96](https://github.com/shaug/atelier/commit/51f6a9631c93f42e59ddb0cbe00287ad9db337d0))
- **gc:** align at:draft normalization reason with deferred lifecycle
  ([8f47c4d](https://github.com/shaug/atelier/commit/8f47c4d61e8992f0ff66ab31cd319c967077543d)),
  closes [#259](https://github.com/shaug/atelier/issues/259)
- **gc:** clarify closed-epic override risk messaging
  ([cd4e9b7](https://github.com/shaug/atelier/commit/cd4e9b70eae04e76f360b131e5c74751c19c9267))
- **gc:** drop at:ready on non-active epic statuses
  ([b0e8d3b](https://github.com/shaug/atelier/commit/b0e8d3be6d4ef47a480eb1db55bf65da12f9bc7f))
- **gc:** fail closed on ambiguous mapping metadata
  ([0fac004](https://github.com/shaug/atelier/commit/0fac0049ef051ebe496ee97317bff3ed990f19b0))
- **gc:** guard stale message cleanup by exact assignee
  ([f98d1ef](https://github.com/shaug/atelier/commit/f98d1efd43176480ba6c96681e6dcc64b71d3b7e))
- **gc:** make closed-unintegrated epic cleanup explicit and recoverable
  ([84a8119](https://github.com/shaug/atelier/commit/84a811940fd3b3a71199733c2ef1652e4a7d423a))
- **gc:** mark dry-run skip diagnostics as report-only
  ([5238f5a](https://github.com/shaug/atelier/commit/5238f5aaacbb164f73a8b5e2e7ac5e180b371b4e))
- **gc:** migrate legacy lifecycle metadata to canonical status
  ([25c0880](https://github.com/shaug/atelier/commit/25c0880afefac5e4fc433d223fe8a80b0bd58301)),
  closes [#259](https://github.com/shaug/atelier/issues/259)
- **gc:** preserve report-only dry-run rendering after rebase
  ([14c9935](https://github.com/shaug/atelier/commit/14c993581af125d32e597f3e494d968d81762590))
- **gc:** prune missing-mapping changeset worktrees before branch cleanup
  ([d03ba0e](https://github.com/shaug/atelier/commit/d03ba0ed038373737deb7944f7d246a29ef6700f))
- **gc:** prune stale session-scoped agent beads
  ([175b918](https://github.com/shaug/atelier/commit/175b918fa56d44cdb761d5398eb147f66802f387))
- **gc:** remove workspace-label requirement for closed branch cleanup
  ([091febc](https://github.com/shaug/atelier/commit/091febcb9090e23eff72e2c04d2db3c530d521c3)),
  closes [#294](https://github.com/shaug/atelier/issues/294)
- **gc:** resolve migrated mapping ownership by metadata
  ([f9b76a5](https://github.com/shaug/atelier/commit/f9b76a518178f0a7906c36516b817f5e71fee3b5))
- **gc:** tolerate missing epic lookups during cleanup
  ([883dd8d](https://github.com/shaug/atelier/commit/883dd8d919f0ce42d81a6d2a2fcdff66c8d86a3c)),
  closes [#293](https://github.com/shaug/atelier/issues/293)
- **init:** add discriminator-based service result checks
  ([267d8e4](https://github.com/shaug/atelier/commit/267d8e47db66dcb8663539b91f68519497d1beed))
- **init:** avoid same-prefix failures during beads prefix migration
  ([b7018d6](https://github.com/shaug/atelier/commit/b7018d6ecd616bf2f1dbfcac82f9daf22245a6b0)),
  closes [#411](https://github.com/shaug/atelier/issues/411)
- **init:** default initialize service run wiring
  ([2b1f1eb](https://github.com/shaug/atelier/commit/2b1f1eb6c2a25017993864ed3c205189144b09c3)),
  closes [#158](https://github.com/shaug/atelier/issues/158)
- **init:** map boundary exceptions to service failures
  ([c4d4cb9](https://github.com/shaug/atelier/commit/c4d4cb976045342c384abd6b381558993200a53f))
- **init:** remove explicit is True result check
  ([4016573](https://github.com/shaug/atelier/commit/4016573e736082ef9d689de050df336f9e685fbf))
- **init:** remove init service gateway indirection
  ([d78d98c](https://github.com/shaug/atelier/commit/d78d98c7d0b31554b07b066fb185502a51fa51bd)),
  closes [#158](https://github.com/shaug/atelier/issues/158)
- **lifecycle:** preserve unknown statuses over label hints
  ([f9b50f9](https://github.com/shaug/atelier/commit/f9b50f9e7e327b57be12b271cf06a5a0009c5928)),
  closes [#255](https://github.com/shaug/atelier/issues/255)
- **lifecycle:** remove remaining cs label compatibility paths
  ([ed86a89](https://github.com/shaug/atelier/commit/ed86a895d13e5e688236a46e1df1090a632796a5))
- **lint:** skip hotspot guardrails when script is absent
  ([72e61b0](https://github.com/shaug/atelier/commit/72e61b0aa9423c9e45c8abf6df0ca8b8c4a1afd2))
- **mail-send:** remove unsupported status flag from reroute epic create
  ([4ce0390](https://github.com/shaug/atelier/commit/4ce0390e9db6840fa39f7a0c68ab818bcaebaca6))
- **normalize-prefix:** replace unsupported beads rollback export command
  ([3c94765](https://github.com/shaug/atelier/commit/3c947654126e2cd92de3102a633b63a9a748178a)),
  closes [#482](https://github.com/shaug/atelier/issues/482)
- **open:** support hooked and changeset branch workspace lookup
  ([7775fe1](https://github.com/shaug/atelier/commit/7775fe1ef4c7e94d51348222b5a134addf45208e))
- **plan-changesets:** guard create against existing changeset mutation
  ([a8072c7](https://github.com/shaug/atelier/commit/a8072c7ca5272a5e9edff0e8beb04046b782f436)),
  closes [#336](https://github.com/shaug/atelier/issues/336)
- **plan-changesets:** include create output excerpts in fail-closed errors
  ([56721f6](https://github.com/shaug/atelier/commit/56721f6ee901b9356b5524c3f84180a6de6188a6))
- **plan-changesets:** list epic children without pagination
  ([b87afd2](https://github.com/shaug/atelier/commit/b87afd2df9abe38aaee6337d9ab2c0988d3d570f))
- **plan:** fall back when claude resume is unavailable
  ([f6b0c20](https://github.com/shaug/atelier/commit/f6b0c208aeaeb84f8ede059839a9716c3fbf8a21))
- **planner-startup:** cap deferred descendant scans
  ([81e05a5](https://github.com/shaug/atelier/commit/81e05a55726ecc5ddf0a1b2b50c19e658ff05071))
- **planner-startup:** harden deterministic fallback rendering
  ([839b93c](https://github.com/shaug/atelier/commit/839b93c923045e56d26fb3c674ff94d17433541f))
- **planner-startup:** include blocked epics in deferred summary
  ([f6fe61f](https://github.com/shaug/atelier/commit/f6fe61ffc4b4aae3bea56d865215de88beb94293))
- **planner-startup:** preserve process-control exceptions
  ([550b4ab](https://github.com/shaug/atelier/commit/550b4abe6688867a7c3520031abdf07768e2270c))
- **planner:** auto-resolve stale needs-decision messages
  ([73c1be8](https://github.com/shaug/atelier/commit/73c1be85e23df4e764e3f2cbd27b8090af138995))
- **planner:** enforce epic identity parity for top-level work
  ([78ef7ee](https://github.com/shaug/atelier/commit/78ef7ee81c7febb3ac792628c9e62883f06baff4)),
  closes [#362](https://github.com/shaug/atelier/issues/362)
- **planner:** fail closed when deferred status updates fail
  ([79703c5](https://github.com/shaug/atelier/commit/79703c530edcca0ff64ad54f7e9892deab87858f)),
  closes [#258](https://github.com/shaug/atelier/issues/258)
- **planning:** block low-information executable work payloads
  ([5073af7](https://github.com/shaug/atelier/commit/5073af7eeac612a8098a7eebfdfb45ba5c12eb9b)),
  closes [#403](https://github.com/shaug/atelier/issues/403)
- **plan:** pass planner workspace UID to resume lookup
  ([0d98623](https://github.com/shaug/atelier/commit/0d98623995de6916f409e925abf4164a4a98fe87))
- **plan:** resume planner sessions for non-codex agents
  ([b359f80](https://github.com/shaug/atelier/commit/b359f80c87245a0d035ee9c679582ffe65500e56))
- **prefix-migration:** gate PR lookup to branch conflicts
  ([e75b1f7](https://github.com/shaug/atelier/commit/e75b1f7d804e3c518cf374933f7df746a9f7fdcd))
- **queue:** deduplicate stale needs-decision queue messages
  ([a604e13](https://github.com/shaug/atelier/commit/a604e13610034937fa872e4f66adde6c09109e8a))
- **reconcile:** refresh PR lookup after transient error
  ([d496709](https://github.com/shaug/atelier/commit/d496709a6cea4153b306c7b09005014c72d99050)),
  closes [#361](https://github.com/shaug/atelier/issues/361)
- remove final label gate and cover planner overview matrix
  ([8d76684](https://github.com/shaug/atelier/commit/8d766847ed830886a63747c1cd9d1830fb39dde4))
- **root-branch:** add epic id suffix to default branch suggestions
  ([6d82e6d](https://github.com/shaug/atelier/commit/6d82e6d8806a019c8181ad449615c72dc85a96c0)),
  closes [#389](https://github.com/shaug/atelier/issues/389)
- **root-branch:** make default epic root branch suggestions collision-proof
  ([d977085](https://github.com/shaug/atelier/commit/d977085f3c24a08be3e7e768fe70eabc959298b9))
- **runtime-env:** honor explicit empty base_env mappings
  ([0c18ffc](https://github.com/shaug/atelier/commit/0c18ffce2292f15f3d2a448aac4711f78e718c35))
- **runtime-env:** warn on legacy ATELIER_WORKSPACE_DIR repo hint fallback
  ([24a255b](https://github.com/shaug/atelier/commit/24a255b925f93fc85acb2b862a437d5120a1d14d))
- **scripts:** use bash -c for install so it inherits process environment
  ([0352a3c](https://github.com/shaug/atelier/commit/0352a3c35761dc46704ce73eb13abe2d5165a7fe))
- **services:** capture init config exceptions in failures
  ([9878d23](https://github.com/shaug/atelier/commit/9878d2333228b4cdb0f042b88985fbebf5032f8e))
- **services:** resolve PR feedback on init service boundaries
  ([79bef90](https://github.com/shaug/atelier/commit/79bef90b890fdfbff7640f44a8232cdd4d8a69b4))
- **skills:** tighten guardrail sync and threshold matching
  ([ad46a65](https://github.com/shaug/atelier/commit/ad46a65eef43f4ce1d998bab4d77b1bee27cd864)),
  closes [#352](https://github.com/shaug/atelier/issues/352)
- **skills:** unify beads root resolution for planner startup scripts
  ([2a5220b](https://github.com/shaug/atelier/commit/2a5220b042edcedefd054af477dd3dc3a37f84c5)),
  closes [#367](https://github.com/shaug/atelier/issues/367)
- **startup:** fail closed when targeted drift metadata lookup fails
  ([df412cd](https://github.com/shaug/atelier/commit/df412cdfb02bd0d113662123a720fd6e2f37d6e8))
- **startup:** inject opening prompts for all planner and worker agents
  ([b1e2e17](https://github.com/shaug/atelier/commit/b1e2e173651bcc2a329c90a0c8c9b688cdd6b011))
- **status:** scope PR payload cache by repo and branch
  ([15a040b](https://github.com/shaug/atelier/commit/15a040b3f9446a134d1349cb77e1846912fc2e80)),
  closes [#260](https://github.com/shaug/atelier/issues/260)
- **templates:** resolve stale packaged template origins
  ([814687a](https://github.com/shaug/atelier/commit/814687aee74fc64eed14b2819e414332ce421883))
- tighten epic parity identity checks for type drift
  ([83d5324](https://github.com/shaug/atelier/commit/83d53246c06ed8e61190a4c9b40ade37caeb0c2b))
- **worker-session:** bound structured stream capture tail
  ([60c31df](https://github.com/shaug/atelier/commit/60c31dfcbe7c2ff1be10700ad536d093a729331e))
- **worker-session:** tighten fallback command and stderr classification
  ([8fb3b2d](https://github.com/shaug/atelier/commit/8fb3b2d2d3d4424cc207d01b1262deb867fb9399)),
  closes [#454](https://github.com/shaug/atelier/issues/454)
- **worker-supervisor:** short-circuit install after update failure
  ([936f12a](https://github.com/shaug/atelier/commit/936f12a121e160f26a14e61540cee7ea89ada52d))
- **worker:** align close guard lifecycle and dry-run recovery
  ([b8f3300](https://github.com/shaug/atelier/commit/b8f3300c9ae5fef1fc1c167e718b4bef98241c03)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** align sequential dependency PR gate diagnostics
  ([ee002b0](https://github.com/shaug/atelier/commit/ee002b0f6ea026b0f3be18678cc3a4b21ae28c01)),
  closes [#260](https://github.com/shaug/atelier/issues/260)
- **worker:** align startup claimability with graph-native issue_type contract
  ([c53c692](https://github.com/shaug/atelier/commit/c53c692c5e749c8cdf712ae5dffa6cafdf7fdba3))
- **worker:** align startup reconcile and status with sequential contract
  ([19e50b5](https://github.com/shaug/atelier/commit/19e50b5b8e0817a21c835f7fc429035759fba780)),
  closes [#239](https://github.com/shaug/atelier/issues/239)
- **worker:** allow strict integration proof via root branch
  ([fe89d30](https://github.com/shaug/atelier/commit/fe89d3039d88fe1866b7ca047d3c352a1b47d5be))
- **worker:** allow target-only integrated sha proof
  ([f699923](https://github.com/shaug/atelier/commit/f699923bf2b81e519cb5ba9b02c9d54293e8fa79))
- **worker:** auto-confirm deterministic bead-suffix root branches
  ([f2586bf](https://github.com/shaug/atelier/commit/f2586bf7941f9a63f563c99c9b7f0456cd020fab)),
  closes [#427](https://github.com/shaug/atelier/issues/427)
- **worker:** auto-reclaim stale explicit epic locks
  ([a72918d](https://github.com/shaug/atelier/commit/a72918d547e962006621574e1312d2b494916d72)),
  closes [#270](https://github.com/shaug/atelier/issues/270)
- **worker:** avoid forced PR refresh in lineage repair lookup
  ([e7e96ba](https://github.com/shaug/atelier/commit/e7e96bae93f4a99e2a395d9f53e86d4186bd5134)),
  closes [#461](https://github.com/shaug/atelier/issues/461)
- **worker:** block premature changeset closure guidance
  ([4bd0ac3](https://github.com/shaug/atelier/commit/4bd0ac37ce7b2e38f0d30ba3a8b8883a2b14d6ea)),
  closes [#376](https://github.com/shaug/atelier/issues/376)
- **worker:** bound pending stream accumulator
  ([b6070ca](https://github.com/shaug/atelier/commit/b6070ca0f14502e1839baa3a151dd28338af0474))
- **worker:** clarify startup finalize-only summary semantics
  ([3ee3160](https://github.com/shaug/atelier/commit/3ee31601940f75013dafb3c3366b69f86fccaec2))
- **worker:** continue implicit runs after finalize decision-required failures
  ([3c4764f](https://github.com/shaug/atelier/commit/3c4764f65dd628f775a04d63843fda5838d434b3)),
  closes [#337](https://github.com/shaug/atelier/issues/337)
- **worker:** continue implicit work after local epic failures
  ([cf7d5c8](https://github.com/shaug/atelier/commit/cf7d5c8fbe597648cd69950efa511b97a80083d9)),
  closes [#297](https://github.com/shaug/atelier/issues/297)
- **worker:** couple in-progress recovery with ticket reopen
  ([9ae4a05](https://github.com/shaug/atelier/commit/9ae4a058677c8d89968e3f9a0f33fff32b2bc313)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** deep-clone runtime args across implicit retries
  ([d5c586c](https://github.com/shaug/atelier/commit/d5c586c255dbb6f3b292c6bdcb86801a66c468cf))
- **worker:** enforce active root branch ownership during startup
  ([dba1c8e](https://github.com/shaug/atelier/commit/dba1c8ee0179423164c3e7558e040cbff8c49704)),
  closes [#249](https://github.com/shaug/atelier/issues/249)
- **worker:** enforce dependency gating for in-progress transitions
  ([e69faed](https://github.com/shaug/atelier/commit/e69faedbf6434b3bf202373a455c2dcd4a3b33db))
- **worker:** enforce deterministic non-watch exit contract
  ([614fd5a](https://github.com/shaug/atelier/commit/614fd5a94a7dff25e30821701c227a6687d99ed9)),
  closes [#227](https://github.com/shaug/atelier/issues/227)
- **worker:** enforce integrated-only sequential dependency checks
  ([382948c](https://github.com/shaug/atelier/commit/382948cd769f217eec8fa07d7c83c3cc54684d74)),
  closes [#242](https://github.com/shaug/atelier/issues/242)
- **worker:** enforce mainline proof for stacked merged PRs
  ([c4c3d12](https://github.com/shaug/atelier/commit/c4c3d121114bb452d8aa2e18377a3fb22bd34271)),
  closes [#495](https://github.com/shaug/atelier/issues/495)
- **worker:** enforce merged close-state integration proof
  ([3506768](https://github.com/shaug/atelier/commit/35067682708cb488e4e58e8580b591e1fd497466))
- **worker:** enforce planner-worker lifecycle boundaries
  ([87b23cf](https://github.com/shaug/atelier/commit/87b23cfed16fcda22e70bffb687405fc3cdeaf6a))
- **worker:** enforce sequential PR base on epic parent branch
  ([db61062](https://github.com/shaug/atelier/commit/db61062f858a14b149e3719198397dceef9dd20b)),
  closes [#321](https://github.com/shaug/atelier/issues/321)
- **worker:** enforce source-lineage gate for strict merged PR proof
  ([8c33716](https://github.com/shaug/atelier/commit/8c33716ffaf5e88eda89c8368431339f351f0cdb)),
  closes [#378](https://github.com/shaug/atelier/issues/378)
- **worker:** enforce Tickets section in generated PR bodies
  ([37fd761](https://github.com/shaug/atelier/commit/37fd7610926f79d7a1fb6893b1c2074f8adaae88)),
  closes [#360](https://github.com/shaug/atelier/issues/360)
- **worker:** fail closed on closed-lifecycle PR anomalies
  ([ab8ac7e](https://github.com/shaug/atelier/commit/ab8ac7e89f6c7dd5c3ce075745defb83e477b0dc))
- **worker:** fail closed on indeterminate PR lifecycle refresh
  ([82d06f1](https://github.com/shaug/atelier/commit/82d06f18f9003e42f5f09fed28266b8b09847dea)),
  closes [#499](https://github.com/shaug/atelier/issues/499)
- **worker:** fail closed on stale external ticket clauses
  ([46acf59](https://github.com/shaug/atelier/commit/46acf59bbbcd2a84f9d1eb30e835bd8cb39768fa))
- **worker:** fail closed startup on migration drift
  ([8965f5a](https://github.com/shaug/atelier/commit/8965f5a32ff5f3cf33a8a0acf55d92575f1e1453)),
  closes [#480](https://github.com/shaug/atelier/issues/480)
- **worker:** fail closed when worker template is missing
  ([59cb3f3](https://github.com/shaug/atelier/commit/59cb3f35dd097cdc54edb85d8948856be7fe3cf1))
- **worker:** fallback to parent when parent_id is null
  ([b3adbdc](https://github.com/shaug/atelier/commit/b3adbdca112dbf94de3152514229d40ef539375c))
- **worker:** gate explicit epic reruns on startup lifecycle
  ([89cbada](https://github.com/shaug/atelier/commit/89cbada8941101645ecba07700656f881aa1afaa))
- **worker:** gate explicit startup short-circuit on leaf role
  ([0109a16](https://github.com/shaug/atelier/commit/0109a16752b0563b1274170ada37455f3c32d935))
- **worker:** gate sequential PR creation by dependency frontier
  ([195829b](https://github.com/shaug/atelier/commit/195829b405d93e8265aa7f9343d17de5883162b6))
- **worker:** gate sequential PRs on integrated dependencies
  ([f821a39](https://github.com/shaug/atelier/commit/f821a395b7718bc49b84d3d8294cd785f720c86b)),
  closes [#243](https://github.com/shaug/atelier/issues/243)
- **worker:** gate startup in-progress transition for follow-up runs
  ([67a3404](https://github.com/shaug/atelier/commit/67a34044fba72595e8ed76fd7b819fb1796fc842)),
  closes [#455](https://github.com/shaug/atelier/issues/455)
- **worker:** guarantee sequential base-policy metadata for child changesets
  ([b0eec7f](https://github.com/shaug/atelier/commit/b0eec7f1160a4fe59f1fd48b458ac6f66de119b0)),
  closes [#338](https://github.com/shaug/atelier/issues/338)
- **worker:** handle PR lookup errors in reconcile drift detection
  ([b93991e](https://github.com/shaug/atelier/commit/b93991eae191b35bb843be0adf1d8ec8217c94ff)),
  closes [#361](https://github.com/shaug/atelier/issues/361)
- **worker:** harden bd issue reads against embedded panic
  ([232f87c](https://github.com/shaug/atelier/commit/232f87cf7f4e1f7a2bf635ea6e916f2b34bd934c)),
  closes [#205](https://github.com/shaug/atelier/issues/205)
- **worker:** harden global startup review scan reads
  ([945657a](https://github.com/shaug/atelier/commit/945657a57c87fe31b91a91cc7987784761098607))
- **worker:** harden sequential dependency base gating
  ([1bd46ed](https://github.com/shaug/atelier/commit/1bd46ed4e1488d31210fed756033a4a6943ab28d)),
  closes [#243](https://github.com/shaug/atelier/issues/243)
- **worker:** harden sequential dependency stack preflight
  ([024fd03](https://github.com/shaug/atelier/commit/024fd0352f37293b08a8ea54477476aab37a682d)),
  closes [#231](https://github.com/shaug/atelier/issues/231)
- **worker:** harden startup recovery for embedded bd panics
  ([b4be05c](https://github.com/shaug/atelier/commit/b4be05c24bbf64f819628da6d5fafd39777025f5)),
  closes [#210](https://github.com/shaug/atelier/issues/210)
- **worker:** honor Beads dependency payloads in startup selection
  ([bc7724f](https://github.com/shaug/atelier/commit/bc7724ff8a4ef9fd5097cafe3a931ed533a61ede)),
  closes [#209](https://github.com/shaug/atelier/issues/209)
- **worker:** honor merged PR merge commit in strict integration proof
  ([b4a98ef](https://github.com/shaug/atelier/commit/b4a98ef6c425ce5c8e21ae973ea629e532ddcaf9)),
  closes [#393](https://github.com/shaug/atelier/issues/393)
- **worker:** honor merged pr_state for sequential startup dependencies
  ([9fe38bd](https://github.com/shaug/atelier/commit/9fe38bdaafdae99e3bc313e7e790f37bd28e029f))
- **worker:** ignore epic parent edges in sequential lineage
  ([d3daf8a](https://github.com/shaug/atelier/commit/d3daf8a85199877ff098349256f7eaff83ad9fdf)),
  closes [#407](https://github.com/shaug/atelier/issues/407)
- **worker:** infer ready fallback epic from graph parent metadata
  ([72f564a](https://github.com/shaug/atelier/commit/72f564a7c4d67314734746062944c993a8f10c01))
- **worker:** infer ready fallback epic from graph parent metadata
  ([bb9bd27](https://github.com/shaug/atelier/commit/bb9bd275eedcd234a55387b5cc71701a96d1dfec))
- **worker:** infer unlabeled dependency issue type in sequential gating
  ([680eabc](https://github.com/shaug/atelier/commit/680eabc55130d6b5a9cdca36a5005cacc4006d9f))
- **worker:** isolate runtime iteration args from caller state
  ([1067d93](https://github.com/shaug/atelier/commit/1067d933acc23b79d2b4858a20dd7d17bb98d279)),
  closes [#337](https://github.com/shaug/atelier/issues/337)
- **worker:** keep default mainline guidance for self-root parent
  ([2bc1a0f](https://github.com/shaug/atelier/commit/2bc1a0fcb3a9ee5e45a1eb7ee45954621d5acad4))
- **worker:** lock parent-child parsing to bd dependency_type
  ([eb2ec13](https://github.com/shaug/atelier/commit/eb2ec13138af284b8647545f0d540a3c128178ca)),
  closes [#260](https://github.com/shaug/atelier/issues/260)
- **worker:** log preserved child workspace parent branch
  ([7e786bb](https://github.com/shaug/atelier/commit/7e786bb256e82448a13f44188361ab9404d61068))
- **worker:** make child parent metadata lookup best-effort
  ([6d9974d](https://github.com/shaug/atelier/commit/6d9974d0179502c57177f8597ce010e8f71a542e)),
  closes [#338](https://github.com/shaug/atelier/issues/338)
- **worker:** make epic startup and status queries unbounded
  ([59f899a](https://github.com/shaug/atelier/commit/59f899a4054fffbdd289fa5e4c47a4dd3a4250d4))
- **worker:** make epic startup and status queries unbounded
  ([9168a5a](https://github.com/shaug/atelier/commit/9168a5a2810a5f062d52642b88e3d57db536e621))
- **worker:** make finalize integrated SHA writes idempotent
  ([9f82a3f](https://github.com/shaug/atelier/commit/9f82a3f93c1f60fe7dc33cfd51fcb7e8efa664b8)),
  closes [#190](https://github.com/shaug/atelier/issues/190)
- **worker:** make finalize PR-base mismatch decisions deterministic
  ([db473dc](https://github.com/shaug/atelier/commit/db473dc4121d891763ceae7a80bdda19140f5e1f)),
  closes [#491](https://github.com/shaug/atelier/issues/491)
- **worker:** make prepare-worktrees startup failures actionable
  ([bae07ef](https://github.com/shaug/atelier/commit/bae07efada8ab9374e821f6fbf518cd297236733)),
  closes [#402](https://github.com/shaug/atelier/issues/402)
- **worker:** narrow deepcopy fallback and log diagnostics
  ([cbfc4c2](https://github.com/shaug/atelier/commit/cbfc4c2c7f8ad039462e0696fe61c717822b19e5))
- **worker:** parse Oxford-comma ticket references
  ([7175b2c](https://github.com/shaug/atelier/commit/7175b2c4097f33436908f0b924e45083ad661303)),
  closes [#360](https://github.com/shaug/atelier/issues/360)
- **worker:** pass resume_review in explicit startup changeset lookup
  ([35fbf6e](https://github.com/shaug/atelier/commit/35fbf6e139aa3e9982046286e96d7320c78f95c1))
- **worker:** persist reconciled lineage after parent integration
  ([907eec0](https://github.com/shaug/atelier/commit/907eec03ed4689b5632baf10788a336709a0e383)),
  closes [#180](https://github.com/shaug/atelier/issues/180)
- **worker:** prefer graph parentage for ready changeset epic mapping
  ([18c81b8](https://github.com/shaug/atelier/commit/18c81b86c0ba415e3232e27cdc52e6151c1ac787)),
  closes [#289](https://github.com/shaug/atelier/issues/289)
- **worker:** prefer state_updated_at for ticket clause freshness
  ([7525c6d](https://github.com/shaug/atelier/commit/7525c6d2bb98096eeaf390ea2d813060c4c76e9f))
- **worker:** preserve beads context in startup PR gating
  ([6637508](https://github.com/shaug/atelier/commit/6637508ad73062dbd6a4859e16e09a19a6f902e4)),
  closes [#358](https://github.com/shaug/atelier/issues/358)
- **worker:** preserve dry-run close readiness signal
  ([8a24a51](https://github.com/shaug/atelier/commit/8a24a51ee7ea57a346983d037bf373be7e369182)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** preserve epic parent lookup in sequential base preflight
  ([2686112](https://github.com/shaug/atelier/commit/26861127a413b325f4fd0936e524e3e33e91e318)),
  closes [#321](https://github.com/shaug/atelier/issues/321)
- **worker:** preserve non-leaf dependency terminal semantics
  ([5959c2c](https://github.com/shaug/atelier/commit/5959c2ca185c412f9b624e01fe1a363652b57d6c))
- **worker:** preserve sequential integration fallback after rebase
  ([3e354cc](https://github.com/shaug/atelier/commit/3e354cc5ba843c23be3411f052830926ac08cabf)),
  closes [#242](https://github.com/shaug/atelier/issues/242)
- **worker:** preserve terminal markers for closed finalize paths
  ([4bcbcfc](https://github.com/shaug/atelier/commit/4bcbcfcd03e88589b9d2ebf8a74174e44026e844))
- **worker:** preserve work-to-root checks for stacked changesets
  ([8629ff8](https://github.com/shaug/atelier/commit/8629ff8aa1be5973f00ce2c406a306f1d9de3ef4))
- **worker:** prevent false merged closure for epic-as-changeset
  ([596fcdf](https://github.com/shaug/atelier/commit/596fcdfb42e441cd23671fc6e6cedd3fadaf8567)),
  closes [#309](https://github.com/shaug/atelier/issues/309)
- **worker:** prevent false missing integration signal on merged PRs
  ([9ee9703](https://github.com/shaug/atelier/commit/9ee970373e85f02bb28f8ede0cbdb2a5b3f0f665)),
  closes [#378](https://github.com/shaug/atelier/issues/378)
- **worker:** prevent premature changeset closure with open PRs
  ([2f17466](https://github.com/shaug/atelier/commit/2f174665edba36cbc7d2e2de51ec3435148ee3ba)),
  closes [#234](https://github.com/shaug/atelier/issues/234)
- **worker:** prevent premature close while PR lifecycle is active
  ([2a266e7](https://github.com/shaug/atelier/commit/2a266e70c257d1ccd09d80979563f613ee5d18bb))
- **worker:** prevent premature close with active PR lifecycle
  ([ca4a2f3](https://github.com/shaug/atelier/commit/ca4a2f3116264dcb6b203503330768a09cf2b29c)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** prevent stale reconcile reopen from stored PR state
  ([9296d2a](https://github.com/shaug/atelier/commit/9296d2a60a611e4014e3c7bde7557ed8e868549a)),
  closes [#361](https://github.com/shaug/atelier/issues/361)
- **worker:** recompute strict integration source refs after fetch
  ([79c3a3b](https://github.com/shaug/atelier/commit/79c3a3b83400db97bfd4d76bac4edccdfcc39d27))
- **worker:** reconcile epic changeset branch metadata at startup
  ([1473a4d](https://github.com/shaug/atelier/commit/1473a4dcf8fe2e44fced3e50080fa85229963a01)),
  closes [#250](https://github.com/shaug/atelier/issues/250)
- **worker:** reconcile explicit-epic no-work exits
  ([6110f68](https://github.com/shaug/atelier/commit/6110f68ae0d0804d6a184f8a949b7668d0f678c0))
- **worker:** reconcile merged PR state during startup and sweep
  ([6555ecb](https://github.com/shaug/atelier/commit/6555ecb8e6eb7122bbc3c8d8510d539698f290e0)),
  closes [#272](https://github.com/shaug/atelier/issues/272)
- **worker:** recover active-pr changesets without stale ticket state
  ([8c677a8](https://github.com/shaug/atelier/commit/8c677a8d39df9f4cb0401af6493f945073a74ade)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** reduce raw agent trace noise in worker sessions
  ([887b4e6](https://github.com/shaug/atelier/commit/887b4e6a05679e45ca068da7c4823b9c0770eba6)),
  closes [#429](https://github.com/shaug/atelier/issues/429)
- **worker:** refresh closed PR lifecycle before escalation
  ([e7f0703](https://github.com/shaug/atelier/commit/e7f0703257561f1f6540c9f5f5d13e6f83cee539))
- **worker:** remove lifecycle-label mutation guidance
  ([0ebebb8](https://github.com/shaug/atelier/commit/0ebebb826af86b1ee25d2fc0b2b137bb66b48527))
- **worker:** remove lifecycle-label mutation guidance
  ([343e665](https://github.com/shaug/atelier/commit/343e66532b3699cf9a107fdc95a4bfbfb1b107ba))
- **worker:** repair non-epic lineage drift before startup checkout
  ([e663916](https://github.com/shaug/atelier/commit/e663916b00b72ed23f310de2bdad1b22e97f1a32))
- **worker:** require at:epic for executable epic identity
  ([ba86b09](https://github.com/shaug/atelier/commit/ba86b0977c3b552d51c84a75ec1e6e7616638da5))
- **worker:** require at:epic for executable epic identity
  ([af818a5](https://github.com/shaug/atelier/commit/af818a5b7b69dd8bcaf3a5b9d8e8a63eab18fb51))
- **worker:** require origin target refs for strict integration proof
  ([605d2be](https://github.com/shaug/atelier/commit/605d2be611844f54561918a9041d59696ca3e212))
- **worker:** require origin-first strict integration proof
  ([2cb1c62](https://github.com/shaug/atelier/commit/2cb1c62b37947f6249ac6dcfbd93b59aee63e424))
- **worker:** require proven dependency integration signals
  ([0ba90dc](https://github.com/shaug/atelier/commit/0ba90dcbbbf2ef24e9eed09c2c7b1c53ec0f782a)),
  closes [#239](https://github.com/shaug/atelier/issues/239)
- **worker:** resolve transitive duplicate dependency parents
  ([8278e19](https://github.com/shaug/atelier/commit/8278e19ac8e7f1ed671502aaa18d4056c8003f92)),
  closes [#185](https://github.com/shaug/atelier/issues/185)
- **worker:** restore ancestor container closure on finalize
  ([615fba7](https://github.com/shaug/atelier/commit/615fba7e3715e718e77588871350f18d5a268fd7)),
  closes [#331](https://github.com/shaug/atelier/issues/331)
- **worker:** restore terminal status constant after rebase
  ([0c31060](https://github.com/shaug/atelier/commit/0c3106056d04eaee234273e8877cf7d58262fc15))
- **worker:** scope command output capture to worktree ops
  ([48d0114](https://github.com/shaug/atelier/commit/48d0114668560a6430596788916c394633766c33))
- **worker:** scope startup drift preflight to selected changeset
  ([122c3b5](https://github.com/shaug/atelier/commit/122c3b5a91e7e999b0b3a2525f3cfcb94a1ab76f)),
  closes [#480](https://github.com/shaug/atelier/issues/480)
- **worker:** skip epic close mutations during startup dry-run
  ([fc1df71](https://github.com/shaug/atelier/commit/fc1df719d173786a93a5f8e554c2dba6085eebf4)),
  closes [#349](https://github.com/shaug/atelier/issues/349)
- **worker:** skip startup finalize mutation during dry-run
  ([27db1d6](https://github.com/shaug/atelier/commit/27db1d69ed936f03bb8dd487aaf95b953586915d))
- **worker:** stabilize review candidate ordering
  ([dcb51b6](https://github.com/shaug/atelier/commit/dcb51b6849bd38a069b6468ab254cc10d31570db))
- **worker:** stop default loop after first PR handoff
  ([bef648e](https://github.com/shaug/atelier/commit/bef648ed8ecc0fdcfc68b419c847e1f4252b3963)),
  closes [#215](https://github.com/shaug/atelier/issues/215)
- **worker:** tighten close-state integration proof checks
  ([5cb0333](https://github.com/shaug/atelier/commit/5cb0333678ecca845c0799ae1a960fcf8fb31a24))
- **worker:** tighten closed-state finalization and dependency gating
  ([9cdf3a3](https://github.com/shaug/atelier/commit/9cdf3a32ae03b5eae70594b5db5e973a4c029aaf))
- **worker:** tighten explicit ticket reference parsing
  ([4161245](https://github.com/shaug/atelier/commit/41612459512dd20c1917a0e7ea28a7c6c87c92f9)),
  closes [#360](https://github.com/shaug/atelier/issues/360)
- **worker:** tighten follow-up dependency gate failures
  ([bacca71](https://github.com/shaug/atelier/commit/bacca71d1cf48db044df797125845dd38d2a878b))
- **worker:** tighten merged integrated_sha fallback lineage proof
  ([813beff](https://github.com/shaug/atelier/commit/813beff01a9a068159f03ba4da8236559242afb0)),
  closes [#378](https://github.com/shaug/atelier/issues/378)
- **worker:** treat type=parent-child as parent-child relation in dependency
  parsing
  ([c769876](https://github.com/shaug/atelier/commit/c76987672ed66e3106b53bef16daf5f696aa160a))
- **worker:** use at:epic with --all for deterministic epic pool enumeration
  ([1b993aa](https://github.com/shaug/atelier/commit/1b993aa6b70ae4c1d14666318386313f1ae1dfca))
- **worker:** use canonical in_progress claims and close markers
  ([d1f1b2d](https://github.com/shaug/atelier/commit/d1f1b2d0ce3e645cb929012dba4f8ea5944502f4))
- **worker:** validate reroute executable payloads
  ([f9d4e62](https://github.com/shaug/atelier/commit/f9d4e6289cb07a3b02c87ff40cf039b2392cebef)),
  closes [#403](https://github.com/shaug/atelier/issues/403)
- **worker:** verify merged integration against refreshed origin refs
  ([029b050](https://github.com/shaug/atelier/commit/029b05092464216e1f0f0811c9a4b95020f36f6c)),
  closes [#355](https://github.com/shaug/atelier/issues/355)
- **workspace:** preserve agent identity in sanitized launch env
  ([a49f559](https://github.com/shaug/atelier/commit/a49f55923601fe01d8f9c3ace2056c36c2ca5911))
- **worktrees:** key state-lock reentrancy by lock path
  ([6f7a4e5](https://github.com/shaug/atelier/commit/6f7a4e54260a4c2caeb510f7176638cf78965e97))
- **worktrees:** preserve migrated mapping path during ownership reconcile
  ([1722d95](https://github.com/shaug/atelier/commit/1722d9567aeaae3a955198cb86cc5ac8886666dc)),
  closes [#438](https://github.com/shaug/atelier/issues/438)
- **worktrees:** reconcile mapping ownership to epic descendants
  ([6656486](https://github.com/shaug/atelier/commit/6656486af99aa9aac36d6ec139e7d6a16a25a998)),
  closes [#248](https://github.com/shaug/atelier/issues/248)
- **worktrees:** repair drifted destination mapping paths
  ([1067055](https://github.com/shaug/atelier/commit/10670555524378a1b055aa8a86f9206c250fb654)),
  closes [#438](https://github.com/shaug/atelier/issues/438)
- **worktrees:** self-heal root-branch mapping migration
  ([6a45085](https://github.com/shaug/atelier/commit/6a45085c54b46996080d0fcf7562aeb54644ae9e))

### Performance Improvements

- **worker:** scale startup epic reconciliation to active work
  ([bf39e57](https://github.com/shaug/atelier/commit/bf39e57d0b07507c482b2faf8a5cda44c58e907b))

## [0.7.0](https://github.com/shaug/atelier/compare/v0.6.0...v0.7.0) (2026-02-22)

### Overview

This release is the pivot from workspace-only tooling to full planning/runtime
orchestration. Atelier adds planner/worker/session isolation, bead-native epic
and changeset workflows, and mail/queue-driven coordination so multi-agent work
can be run with explicit lifecycle state. It also deepens PR-aware publishing
with strategy gating, review feedback handling, external ticket integration, and
heavy reliability hardening around reconcile/finalization flows.

### Features

- add on-demand planner startup overview refresh
  ([f0405ef](https://github.com/shaug/atelier/commit/f0405efcc0ad269a88b74baf880d1e1916971687))
- **agent-home:** isolate planner and worker sessions
  ([c333506](https://github.com/shaug/atelier/commit/c333506e3550b2831f2dd64a52250208d4f077eb))
- **agent:** add agent home directories
  ([1f79f38](https://github.com/shaug/atelier/commit/1f79f38060e4dac329ee42fb100d797db8b132b1))
- **agent:** add identity config and bead helpers
  ([250e889](https://github.com/shaug/atelier/commit/250e889aff04274dca0dbe8f9eae5ab2198bab85))
- **agent:** inject bd prime addendum into generated AGENTS.md
  ([34c353f](https://github.com/shaug/atelier/commit/34c353f06a81573d85d263d70826188ced68052f))
- **agents:** inject identity env vars
  ([f450338](https://github.com/shaug/atelier/commit/f4503385ed75d58f0b657719c2b86e28292e498f))
- **agents:** scope identity env vars
  ([cc4840f](https://github.com/shaug/atelier/commit/cc4840fa330a2e2f502d322225c4b9102738bee6))
- **beads:** add epic claim and agent hook
  ([74548cc](https://github.com/shaug/atelier/commit/74548cc8628752980bdf407ae84ff75311ce0061))
- **beads:** add location discovery
  ([8d3ceef](https://github.com/shaug/atelier/commit/8d3ceef75af0f7aed954004cb232368f09f576a1))
- **beads:** add message frontmatter helpers
  ([b7babaa](https://github.com/shaug/atelier/commit/b7babaa6ff9979b2b8b409c15f4778824dc6bbe5))
- **beads:** backfill hook slots
  ([7de5bee](https://github.com/shaug/atelier/commit/7de5beebf88c63d0afccdf36df40469a4e697538))
- **beads:** close epics when complete
  ([95a1d97](https://github.com/shaug/atelier/commit/95a1d974955a351fe78a8a52870f0b9d3ceca3f0))
- **beads:** detect epic completion
  ([9fcc799](https://github.com/shaug/atelier/commit/9fcc79953d25a505bd55bece2070ae5a03adf19d))
- **beads:** isolate atelier planning store
  ([d4fd568](https://github.com/shaug/atelier/commit/d4fd56828e79e5718ad99077958867bdcd19ac50))
- **beads:** store hooks in slots
  ([9d0dea6](https://github.com/shaug/atelier/commit/9d0dea6d878f75f813d46ced2502bd4c6243ec5a))
- **changesets:** add review metadata helpers
  ([0619ead](https://github.com/shaug/atelier/commit/0619ead521b154358e273024f6a26fce06a0d613))
- **changesets:** add review update skill
  ([62dbce4](https://github.com/shaug/atelier/commit/62dbce45676c3f00b9c0ef844f344d13e211e057))
- **changesets:** derive lifecycle labels from PR state
  ([7481bef](https://github.com/shaug/atelier/commit/7481befb8a7120b15c83793929e69352d08968f0))
- **cli:** add mail command
  ([e885271](https://github.com/shaug/atelier/commit/e8852719cb005e3d7cd929681b49788494be65fc))
- **cli:** add plan/work entrypoints
  ([e6216b1](https://github.com/shaug/atelier/commit/e6216b1dea1210ddf4d653fa30d40bb58747f693))
- **cli:** add remove command for deleting project state
  ([b0e8d20](https://github.com/shaug/atelier/commit/b0e8d20193ec2b0e14a75ea96c48311ed354aa02))
- **cli:** drop legacy commands and add gc
  ([4bcfb82](https://github.com/shaug/atelier/commit/4bcfb822e1583f7a19b038498d5aef96616bc196))
- **cli:** improve worker logging and epic promotion
  ([7fe310e](https://github.com/shaug/atelier/commit/7fe310e1d407f1b87bd61e8f98308ab4db957cef))
- **cli:** replace describe with status
  ([460f50a](https://github.com/shaug/atelier/commit/460f50a9e971eb9c54906ccc7ea098beccca9b99))
- **config:** add pr strategy config and gating
  ([8dc8cd5](https://github.com/shaug/atelier/commit/8dc8cd5fc10a3aaa4cb517c098ea19a1ca8ec8bb))
- **config:** record project data dir
  ([aff65ff](https://github.com/shaug/atelier/commit/aff65ff663f3a7afa0f582d415ade8a71fc8f1d4))
- **core:** expand beads metadata and queue handling
  ([f94ea37](https://github.com/shaug/atelier/commit/f94ea376b3673a5269f8a957843e1dc192d062c8))
- **daemon:** add full-stack daemon support
  ([fa7a5d7](https://github.com/shaug/atelier/commit/fa7a5d7c36b4875a642fec9a1d5bb097392fa677))
- **epic-list:** surface dependency-blocked epics and blocker ids
  ([d80807d](https://github.com/shaug/atelier/commit/d80807d403d23a7154d6c570a84e502916703f72))
- **external-providers:** add optional sync toggles
  ([e70b94f](https://github.com/shaug/atelier/commit/e70b94f0987271f276296e2166a9691501c49a4e))
- **external-registry:** auto-discover repo beads
  ([c765f1c](https://github.com/shaug/atelier/commit/c765f1c7559f63dd7e62033fadd4aa53d4a31389))
- **external-tickets:** add enrichment fields
  ([434fb03](https://github.com/shaug/atelier/commit/434fb039a34db1fddce64d15ff02b964b557d932))
- **external-tickets:** add provider contract scaffolding
  ([a2f6f36](https://github.com/shaug/atelier/commit/a2f6f36146ee96884585297ad7ea51c52d346524))
- **external-tickets:** add schema normalization helpers
  ([f381a95](https://github.com/shaug/atelier/commit/f381a95096f31ce19ea5d4935e5d38e069b39a6f))
- **external:** add optional default auto-export for new planning beads
  ([6945f2c](https://github.com/shaug/atelier/commit/6945f2c569e446fd292589c508d72d1798998fc8))
- **gc:** add detailed reconcile and cleanup logging
  ([939cedc](https://github.com/shaug/atelier/commit/939cedccc938add86dc3853f6b9343ab99881da1))
- **gc:** prune resolved epic artifacts after integration is proven
  ([30e23a5](https://github.com/shaug/atelier/commit/30e23a50efa2224a0a07691c320280ef21401fd7))
- **gc:** release expired and closed hooks
  ([afcf188](https://github.com/shaug/atelier/commit/afcf188b20c1e1b5b681227996716d5cf3a694b9))
- **gc:** release stale queue claims
  ([199ecec](https://github.com/shaug/atelier/commit/199ecec247be2c74e7d57cef5bd3b4bdb3c19843))
- **github-issues:** add provider adapter
  ([62003cf](https://github.com/shaug/atelier/commit/62003cf9eab1f8956542beec2d298e143437353b))
- **hooks:** add hook adapter
  ([13297e8](https://github.com/shaug/atelier/commit/13297e878172d63938450afbd01a67a9a0fe14c3))
- **init:** re-run setup and ensure beads types
  ([381fc41](https://github.com/shaug/atelier/commit/381fc4196097bae5aac5e0541442be6eaee1af28))
- **init:** reconcile managed skills and fix provider prompts
  ([89cc91f](https://github.com/shaug/atelier/commit/89cc91fdff34165af05650735ab171cd6b3ce296))
- **integration:** add cas changeset integration
  ([9317261](https://github.com/shaug/atelier/commit/9317261edfae2f13182893465b2f90e8f80de58f))
- **mail:** add message bead helpers
  ([b4b9632](https://github.com/shaug/atelier/commit/b4b963267ab08c9d53788e8dcb8253a174ede8d7))
- **paths:** add project data dir helpers
  ([5a5517e](https://github.com/shaug/atelier/commit/5a5517e24f694753a83ce2fe1e534d6f3f76eb30))
- **plan:** add interactive planner flow
  ([b2924e8](https://github.com/shaug/atelier/commit/b2924e888b3572e6dc01e618d5524fe56c9277c8))
- **plan:** add on-demand planner startup overview
  ([ee23559](https://github.com/shaug/atelier/commit/ee235598e052dcec8f3db592e87bc8165ba3bfe7))
- **plan:** add planner guardrails
  ([c400678](https://github.com/shaug/atelier/commit/c400678787e59330d05a4936e2cd07d7313b8d65))
- **plan:** add planner template variables
  ([e6258a4](https://github.com/shaug/atelier/commit/e6258a4c58a72901b9d0f76522cd30d50f6b7b12))
- **plan:** add startup progress timing
  ([cc5c1fd](https://github.com/shaug/atelier/commit/cc5c1fd1530f9b9cf6c37bba334e215d85608dcb))
- **plan:** enforce planner read-only guardrails
  ([a8164ca](https://github.com/shaug/atelier/commit/a8164caea54ff5a40483b96b7b6c014918c654a8))
- **planner:** keep planner worktrees synced during active sessions
  ([3bb1461](https://github.com/shaug/atelier/commit/3bb1461fa73d1fc9bce5f40e8519bf33afcaab12)),
  closes [#99](https://github.com/shaug/atelier/issues/99)
- **planner:** surface external provider context
  ([47d190a](https://github.com/shaug/atelier/commit/47d190a24ed65335454a5975211023a7b289b1b4))
- **planning:** enforce epic-as-changeset guardrails
  ([ef6f03f](https://github.com/shaug/atelier/commit/ef6f03f5c0410c89721e8c9efd6c09028f0d5e91)),
  closes [#94](https://github.com/shaug/atelier/issues/94)
- **plan:** render planner agents template
  ([be8b7db](https://github.com/shaug/atelier/commit/be8b7db4d32b92572c02b9e1da9709b63c837363))
- **plan:** render planner agents template
  ([8fe522c](https://github.com/shaug/atelier/commit/8fe522cf5817bf6269af4ae3f7bbff3cf4ab9517))
- **plan:** start planner agent sessions
  ([2d7a214](https://github.com/shaug/atelier/commit/2d7a214a43beb11e00c0b6ea6273124c685fe9e6))
- **policy:** add role-scoped project policy beads
  ([fc9ab89](https://github.com/shaug/atelier/commit/fc9ab898784722bac7c375ae48371873a67fd447))
- **policy:** show policy by default and gate edits behind --edit
  ([68edd9f](https://github.com/shaug/atelier/commit/68edd9fc5dd09c60ee22d598a0e49cea344f09c7))
- **pr-strategy:** add on-parent-approved gating mode
  ([f2b2b82](https://github.com/shaug/atelier/commit/f2b2b82c53ac17986a0dcdb59e2b1ad6a9164b0b))
- **pr-strategy:** implement on-ready gating distinct from parallel
  ([5048e82](https://github.com/shaug/atelier/commit/5048e8256a78fa84f54ecdfec70e470ba91160e9))
- **publish:** add external ticket section to generated PR bodies
  ([b2369bb](https://github.com/shaug/atelier/commit/b2369bbec70d4ec5527c5120888f538aa9765b1a))
- **queue:** guide queue intake
  ([78a07f3](https://github.com/shaug/atelier/commit/78a07f356300a83f801f43fcb08db046856f0007))
- **repo-beads:** add export guardrails
  ([027ede2](https://github.com/shaug/atelier/commit/027ede2b01a4cd289c11dc3afd0a3792cf78e578))
- **repo-beads:** add read-only provider
  ([1501014](https://github.com/shaug/atelier/commit/1501014a43cabfbd1bf119810966783fd94d819c))
- **skills:** add agent hook workflows
  ([b8f8cdb](https://github.com/shaug/atelier/commit/b8f8cdbb356891c563e061eb8225e7d4df338da6))
- **skills:** add AgentSkills frontmatter validation gate
  ([17786be](https://github.com/shaug/atelier/commit/17786be5f53702ea3757c5633810f6421b2f3e00))
- **skills:** add beads skill scaffolding
  ([4a41b18](https://github.com/shaug/atelier/commit/4a41b18903cc39b1e7baf0251e720304bc76d299))
- **skills:** add changeset guardrail validation
  ([514f65c](https://github.com/shaug/atelier/commit/514f65c983144aed35510d163f064bec9e01f03c))
- **skills:** add epic list and claim
  ([5844707](https://github.com/shaug/atelier/commit/5844707aeada1f2d62e92359ae69b5a4a6464eeb))
- **skills:** add epic promotion skill
  ([1189c4d](https://github.com/shaug/atelier/commit/1189c4da81286accf8ef8c7be3c16e5e4d6046f7))
- **skills:** add github issues list support
  ([6d2b0d5](https://github.com/shaug/atelier/commit/6d2b0d5767094f53ae218e06063ce0845f3b5508))
- **skills:** add github-issues skill
  ([8199d77](https://github.com/shaug/atelier/commit/8199d7744fb0fff6baffeb138606def058928155)),
  closes [#88](https://github.com/shaug/atelier/issues/88)
- **skills:** add github-prs skill
  ([66b23fe](https://github.com/shaug/atelier/commit/66b23fed70edc8d29cb1cbe693d4eef0b9636795)),
  closes [#89](https://github.com/shaug/atelier/issues/89)
- **skills:** add plan_changesets
  ([450026e](https://github.com/shaug/atelier/commit/450026e6ebb43b4116745e011b64abd4d5bfb9a6))
- **skills:** add plan_create_epic
  ([558e553](https://github.com/shaug/atelier/commit/558e5535f0fee01ac555d76f1612a08aeb6d17bf))
- **skills:** add plan_split_tasks
  ([8d4c890](https://github.com/shaug/atelier/commit/8d4c890638de60ba8c5b5544b1c449d41c4f6682))
- **skills:** add planner startup check
  ([8771e7e](https://github.com/shaug/atelier/commit/8771e7e7fc3413082e098bd38eac5d40eb5bc852))
- **skills:** add startup contract
  ([464665e](https://github.com/shaug/atelier/commit/464665e70a7361d99d59b537623a2a3c6f203280))
- **skills:** add workspace-managed skills
  ([d928c5c](https://github.com/shaug/atelier/commit/d928c5c0776e38be32d1d987ec0ce86739d58f60))
- **skills:** reauthor publish workflow
  ([b14428d](https://github.com/shaug/atelier/commit/b14428d4b99889d7f07581e5f569c4e7f07a5077))
- **skills:** reauthor tickets skill
  ([f0d31b3](https://github.com/shaug/atelier/commit/f0d31b3b0a69ce624e3e94274b50583b5ffcac5f)),
  closes [#87](https://github.com/shaug/atelier/issues/87)
- **status:** add PR strategy + draft skill
  ([f80f05e](https://github.com/shaug/atelier/commit/f80f05eb07407c7d9def420e789c9e0e3c3bf26a))
- **status:** add pr strategy gating details
  ([1fcc98d](https://github.com/shaug/atelier/commit/1fcc98da3f8b7d8095a004915b4e7d7a65519ca6))
- **status:** honor cs:ready label
  ([712475b](https://github.com/shaug/atelier/commit/712475b77a243c021b53ce25661e599c80a968a7))
- **status:** report session liveness and reclaimability
  ([0c7d260](https://github.com/shaug/atelier/commit/0c7d2607ad046765ffaace7a7cf95c40b2192359))
- **status:** surface PR lifecycle signals
  ([7690bcc](https://github.com/shaug/atelier/commit/7690bcc2eecfc74cd9bfa6ec2d16d41a729fd903))
- **ticketing:** add skill-based provider detection and planner selection
  ([017efd7](https://github.com/shaug/atelier/commit/017efd7c1bdb1e879e83cceeeb3c37f9d9453edb))
- **work:** add --yes for default prompt choices
  ([0118973](https://github.com/shaug/atelier/commit/011897371ef659d08f142bfa69f2bf069063d076))
- **work:** add deterministic env-to-cli default resolver
  ([9558b0b](https://github.com/shaug/atelier/commit/9558b0bd51da4cb32ed17ab3ffd4579d0dac3a24))
- **work:** add dry-run mode for worker sessions
  ([7bedd50](https://github.com/shaug/atelier/commit/7bedd50318081357b39955b3798171cb9151fcce))
- **work:** add worker guardrails
  ([42363a6](https://github.com/shaug/atelier/commit/42363a6e8040937e150f392fe2ce5414509d29f0))
- **work:** add worker run modes
  ([960451d](https://github.com/shaug/atelier/commit/960451d68b84f6aeef5a33d8b34902c1ac3281eb))
- **work:** add worktree mapping
  ([87a1ff1](https://github.com/shaug/atelier/commit/87a1ff10179d751724a80c59c4cab9e69de21042))
- **work:** allow resume epics in prompt mode
  ([8e0b599](https://github.com/shaug/atelier/commit/8e0b599c4a760291e3625cc8d3c42ddefe8d77ba))
- **work:** check queues before claiming
  ([c2c828d](https://github.com/shaug/atelier/commit/c2c828dbb144704cf4278b590032f645214be141))
- **work:** emit per-changeset reconciliation logs
  ([535e486](https://github.com/shaug/atelier/commit/535e48688bd86ac6e64190ec085d5c13a3b48bc4))
- **work:** enforce epic claim and changeset readiness
  ([ec5c78f](https://github.com/shaug/atelier/commit/ec5c78fae2f588a420dda1a50351d662e3ef37a8))
- **work:** enforce worker completion guardrails
  ([1363116](https://github.com/shaug/atelier/commit/13631163625da006cc618466e86348d01d9821ca))
- **worker:** allow stacked intra-epic changeset selection after review handoff
  ([6b1cc9c](https://github.com/shaug/atelier/commit/6b1cc9ce6bcef49515a479696433268b472ff806)),
  closes [#93](https://github.com/shaug/atelier/issues/93)
- **worker:** enforce planner non-ownership on executable beads
  ([186ac40](https://github.com/shaug/atelier/commit/186ac4024c47683100edaaf6db38eebd99f02e77)),
  closes [#96](https://github.com/shaug/atelier/issues/96)
- **work:** fallback to unfinished epics in auto mode
  ([0c48167](https://github.com/shaug/atelier/commit/0c48167732036816d0dc1494cdcc8d13c42b6296))
- **work:** gate claims on inbox
  ([4b4b2e5](https://github.com/shaug/atelier/commit/4b4b2e5685d2b13f6389cfc9cbc80502a379ceec))
- **work:** launch agents with identity env
  ([218404c](https://github.com/shaug/atelier/commit/218404c09ea51c8dbb8ff2e9f2f8ad954780718b))
- **work:** notify overseer when idle
  ([d54b510](https://github.com/shaug/atelier/commit/d54b510bfcccfa1b9e7bf1ce36c147b2168ec280))
- **work:** prime beads before claiming
  ([02dc8c2](https://github.com/shaug/atelier/commit/02dc8c26007b4e5c8e26b0fc610e96bcd6cc7b06))
- **work:** prioritize review feedback and harden publish finalization
  ([e75d673](https://github.com/shaug/atelier/commit/e75d6734046e53890ed25d680a5cf48f3bc33926))
- **work:** reconcile blocked merged changesets on startup
  ([a0245c1](https://github.com/shaug/atelier/commit/a0245c1735d67e419011f2967af8e1e995ac10b2))
- **work:** record branch metadata
  ([9a190b5](https://github.com/shaug/atelier/commit/9a190b55e013cfee68f364257a35569be3ac9c94))
- **work:** render worker agents template
  ([c34c717](https://github.com/shaug/atelier/commit/c34c7170d04a533b4a3b3261bdf5ae4ed434d7c2))
- **work:** render worker agents template
  ([f6c733b](https://github.com/shaug/atelier/commit/f6c733bc198c93680698e2e8bccb06e788734687))
- **work:** resume hooked epics
  ([4063fc3](https://github.com/shaug/atelier/commit/4063fc38fedb73ef6800de933a195beba30997e6))
- **work:** run startup contract selection
  ([e1ecc8d](https://github.com/shaug/atelier/commit/e1ecc8da7912ff2ce12b8f85702969c8621e6be4))
- **workspaces:** pivot to epic-rooted worktrees
  ([e78edf5](https://github.com/shaug/atelier/commit/e78edf58db0583b63f172df650a49153b493ffce))
- **work:** support agent-generated squash commit subjects
  ([6384a81](https://github.com/shaug/atelier/commit/6384a8132ceccd2752666f21aed9b14425ff08ba))
- **work:** support merge and squash epic finalization
  ([89b0842](https://github.com/shaug/atelier/commit/89b084201fd85789947a810787e207b8322b0716))
- **worktrees:** add changeset worktrees
  ([098a8df](https://github.com/shaug/atelier/commit/098a8df36e9b5b06a22756753b14ced0e27f122f))
- **worktrees:** add git worktree creation
  ([f315d93](https://github.com/shaug/atelier/commit/f315d93960df838e5da08d3d8d9331ec2ba4f414))
- **worktrees:** add removal helper
  ([b594e2e](https://github.com/shaug/atelier/commit/b594e2ec95e83663c8197a90350ae39d30091ae1))
- **worktrees:** checkout changeset branches
  ([96180a1](https://github.com/shaug/atelier/commit/96180a12100c4e16109311bad2fc064df0ec1f59))
- **work:** use interactive epic selection list
  ([30a61f9](https://github.com/shaug/atelier/commit/30a61f9a4b458431e29284fe4e21ac5d1290113a))

### Bug Fixes

- **agent-home:** clean up session homes on exit and gc
  ([a8270dc](https://github.com/shaug/atelier/commit/a8270dc083bae927a1a96be2a29297be8f2bbdae))
- **agent:** add claude compat files
  ([daf4b30](https://github.com/shaug/atelier/commit/daf4b30bdba1b17cec95220e0c610ca83b50f51f))
- **agent:** keep atelier state out of worktrees
  ([b9e2fd6](https://github.com/shaug/atelier/commit/b9e2fd67825699f44e220e59369df62eea76e4cd))
- **ci:** pin ruff and align formatting output
  ([d39ec61](https://github.com/shaug/atelier/commit/d39ec611e8b7dcebbe6de518dda103f3a79d331f))
- **cli:** add global log and color flags
  ([b8c70a2](https://github.com/shaug/atelier/commit/b8c70a24298300023280147e93a6cedde3c64d15))
- **cli:** scope workspace completion
  ([86e1c91](https://github.com/shaug/atelier/commit/86e1c91c55e8564ca4b1ebb2e0120175459b7ddf))
- **gc:** handle dirty orphaned worktrees interactively
  ([47c23a3](https://github.com/shaug/atelier/commit/47c23a340471e4829577c600aba47ab729cbbdf3))
- **gc:** prompt before reconcile actions without --yes
  ([59cac88](https://github.com/shaug/atelier/commit/59cac8818497e13637a327119448c1e28e55d5f8))
- **gc:** prune closed epic artifacts without summary gating
  ([fdb15d7](https://github.com/shaug/atelier/commit/fdb15d7275ceac83bd98ff3898f271412d40106b))
- **gc:** prune integrated workspace branches without mapping metadata
  ([24536e9](https://github.com/shaug/atelier/commit/24536e976f38d052e2a44ce5850af17ce9cd1aec))
- **gc:** show reconcile cleanup targets and execution logs
  ([1880d51](https://github.com/shaug/atelier/commit/1880d515af8dce15564b4a368c8c5f2a19814ca2))
- **hooks:** skip unwritable hook paths
  ([1c3e634](https://github.com/shaug/atelier/commit/1c3e6346bae10f34e6b52e97584f323072ecdcc4))
- **init:** always prompt for external provider strategy in interactive init
  ([1a4c0ee](https://github.com/shaug/atelier/commit/1a4c0ee7a185469b2701d13c32c9c0fd920940d2))
- **init:** choose and persist external provider strategy during initialization
  ([4071afb](https://github.com/shaug/atelier/commit/4071afb2c82b78f21ba6a8743361fedbfd7a625c))
- **init:** keep enlistment repo untouched during project setup
  ([71c93be](https://github.com/shaug/atelier/commit/71c93be6cedc254ce247fdeb235e146901e8b7be))
- **lifecycle:** normalize runtime and gc changeset invariants
  ([fb4bcb9](https://github.com/shaug/atelier/commit/fb4bcb908dfdee38f8084786d0b0b77aa1fab79c))
- **open:** avoid git network hangs
  ([7ffebf4](https://github.com/shaug/atelier/commit/7ffebf40f3552af0bf31ee66d75afa75886c21ad))
- **plan:** keep planner worktree off default branch
  ([ce2dea8](https://github.com/shaug/atelier/commit/ce2dea89db61c409e2886425a2a98f6586e5b329))
- **plan:** keep startup overview refresh inside planner skill
  ([04c5e8e](https://github.com/shaug/atelier/commit/04c5e8eaa46bc1cd2e8660c8f024134ffc881e71))
- **planner:** list active epics by state in epic_list output
  ([c9cbb4a](https://github.com/shaug/atelier/commit/c9cbb4adc664b451163d89ef98ab06c8f3d1cfdd))
- **planner:** reroute inactive worker mail dispatch
  ([6264d90](https://github.com/shaug/atelier/commit/6264d902e609487aa717e6a16d45d290084bc56e))
- **plan:** prompt on dirty planner migration
  ([e28e26b](https://github.com/shaug/atelier/commit/e28e26bab05b889a734952fce283da5b6fa39701))
- **plan:** scope planner commit hook to planner worktree
  ([2a1bb66](https://github.com/shaug/atelier/commit/2a1bb661d69242d142963bc128023fbb3548bc5a))
- **reconcile:** gate plan/work reconcile and scope gc prompts to actionable
  epics
  ([b021bc8](https://github.com/shaug/atelier/commit/b021bc8a4b70415ce455fda5b2356885f6d097b8))
- **reconcile:** include closed epics pending final integration
  ([fc7f670](https://github.com/shaug/atelier/commit/fc7f6702adf8ba2fa0b2ba3f2c35779c44f8ea72))
- **repo-beads:** resolve lint/test issues
  ([2a78f46](https://github.com/shaug/atelier/commit/2a78f46aa6fa304bccc5bdef1943701de6317c76))
- **repo:** address PR review feedback on hook bootstrap
  ([bac4204](https://github.com/shaug/atelier/commit/bac4204e3b5dd6bd04f9355dc6b13d2ca6aa7be3))
- **session:** harden stale worker detection against pid reuse
  ([80f1a17](https://github.com/shaug/atelier/commit/80f1a17927ca370155540caf0cd174a010f55515))
- **skills:** add frontmatter to github skill doc
  ([8774257](https://github.com/shaug/atelier/commit/87742572bea57e15ecdd79cdff3652b63efbd930))
- **skills:** enforce strict AgentSkills validation gate
  ([bdcb312](https://github.com/shaug/atelier/commit/bdcb3121b90cb7d8902bce6b906fc20d6b93210a)),
  closes [#109](https://github.com/shaug/atelier/issues/109)
- **skills:** remove unsupported Actor.isBot field from review thread query
  ([d4e2a27](https://github.com/shaug/atelier/commit/d4e2a27f949a5c7d014cf7430b50eee48a0fcd49))
- **ticketing:** detect legacy provider skills without manifests
  ([4538ebd](https://github.com/shaug/atelier/commit/4538ebd230c509b3976647eacdb3a61be0fceee0))
- **ticketing:** use known provider skills instead of manifest files
  ([4143e0d](https://github.com/shaug/atelier/commit/4143e0d3da605560100a2f1aa1bd1acdb4149e30))
- **work:** accept git-graph integration signals for merged changesets
  ([8066af1](https://github.com/shaug/atelier/commit/8066af100bd52d2b071e2ab5ace06f4dedb773a2))
- **work:** accept integration sha from bead notes
  ([c735ea4](https://github.com/shaug/atelier/commit/c735ea49936a2b0d10ae4a550bcdbe6d42f9e1bb))
- **work:** allow direct epic execution without child changesets
  ([4ad0bc2](https://github.com/shaug/atelier/commit/4ad0bc269c2d028c0683c820dc3f02386b280f42))
- **work:** auto-sync managed skills and finalize pushed changesets
  ([5122249](https://github.com/shaug/atelier/commit/5122249ed80fc36bb54038ac325ab9c50dd03a80))
- **work:** classify pushed-without-pr finalize failures
  ([a737ae3](https://github.com/shaug/atelier/commit/a737ae32c627eaa2419a4ceccd45b6ec1af6bde7))
- **work:** continue past review-pending in-progress changesets
  ([8af70ab](https://github.com/shaug/atelier/commit/8af70abee661f0497f5c9b12bc8f07c8760d5606))
- **work:** create missing PRs during worker finalization
  ([a29a696](https://github.com/shaug/atelier/commit/a29a696ebbf195a147d6b81900677b032b06b9a1))
- **work:** derive PR base from epic parent for first changesets
  ([d317a0a](https://github.com/shaug/atelier/commit/d317a0ad4aacb9a94373864a3bfa9b4aadabd390))
- **work:** derive top-level PR base from workspace parent branch
  ([f790b08](https://github.com/shaug/atelier/commit/f790b08b6f45524c724573454518f00ddeeb778b))
- **work:** detect inline PR comments in feedback prioritization
  ([f20c0b2](https://github.com/shaug/atelier/commit/f20c0b2c6c1d7a112d7d8286f73a9245dae65e07))
- **work:** detect integration sha in notes payloads
  ([7f8fb5b](https://github.com/shaug/atelier/commit/7f8fb5b459bd0237c5a2f14b5d8bde46332f85f9))
- **work:** distinguish missing PRs from PR query failures
  ([ed195bc](https://github.com/shaug/atelier/commit/ed195bc1273ad198bd27f79e128dcace6afd447e))
- **work:** enforce feedback progress before completing review runs
  ([ee03b01](https://github.com/shaug/atelier/commit/ee03b01b59dafa0cdceee0f5b7c3637a89af0a43))
- **work:** enforce inline review-thread replies in feedback runs
  ([a02591e](https://github.com/shaug/atelier/commit/a02591eeebd23836bd1f3a76c7b062783289ae34))
- **work:** enforce pr-required publish lifecycle
  ([6d0c5a4](https://github.com/shaug/atelier/commit/6d0c5a4c0f7569bc0099498a27d8f68f912f6af8))
- **work:** enforce startup feedback and work precedence
  ([e0734f7](https://github.com/shaug/atelier/commit/e0734f701504f0dc3084b2a7665b6daf0a450bbe))
- **worker:** align finalize PR-create callback contract
  ([b351206](https://github.com/shaug/atelier/commit/b3512060e34c6ff19b3e2fc1213c96f73e4a792c))
- **worker:** align PR base to parent lineage
  ([42b1580](https://github.com/shaug/atelier/commit/42b158009a756686892844f69ac612e50772e485)),
  closes [#112](https://github.com/shaug/atelier/issues/112)
- **worker:** avoid failing on stable changeset base metadata
  ([082e150](https://github.com/shaug/atelier/commit/082e15074e8bb4bc608b3fa1c2135bf9524aa74e))
- **worker:** avoid false review-feedback stalls
  ([049543b](https://github.com/shaug/atelier/commit/049543b007c8d6375e93075400cfbd1c06af3b28))
- **worker:** detect default-branch merge conflicts in review checks
  ([b756635](https://github.com/shaug/atelier/commit/b756635966461ceb8e73203a3376e9c521b91531))
- **worker:** harden helper module compatibility lookups
  ([eb195df](https://github.com/shaug/atelier/commit/eb195df4c014f80c805856717ba52fbc8eb021e2))
- **worker:** harden sequential PR gating lineage resolution
  ([a18ea81](https://github.com/shaug/atelier/commit/a18ea81aae466e3276087a30e7e65789562bbc33)),
  closes [#129](https://github.com/shaug/atelier/issues/129)
- **worker:** hydrate changesets before review-feedback selection
  ([d520d5e](https://github.com/shaug/atelier/commit/d520d5e70a1435908f05be58d71fcfc2547e9a03))
- **worker:** restore watch interval default constant
  ([faf8141](https://github.com/shaug/atelier/commit/faf814144319124dd0e801222063db590cc4edc4))
- **worker:** skip unclaimable review feedback epics
  ([6d3df01](https://github.com/shaug/atelier/commit/6d3df0185e7713026e5bcd294fc9e059438fea99))
- **work:** fall back to global ready changesets
  ([b7798e7](https://github.com/shaug/atelier/commit/b7798e79690b5d1cf0519052a6825fe69f8844db))
- **work:** finalize epic integration from owning worktree context
  ([1b8f5a0](https://github.com/shaug/atelier/commit/1b8f5a07d46d75680cf161666a629ee7f416e00f))
- **work:** finalize non-pr epics to parent branch
  ([f5f46b8](https://github.com/shaug/atelier/commit/f5f46b8920b4067149a47d3cb086b65696c62081))
- **work:** harden worker lifecycle and changeset validation
  ([0713af4](https://github.com/shaug/atelier/commit/0713af408385e448291699316e90d76320c869af))
- **work:** include global review-feedback candidates in startup
  ([7d3cb1c](https://github.com/shaug/atelier/commit/7d3cb1c439734e046fae1fda03811d86c5d79c12))
- **work:** keep parent changesets open while subtasks remain
  ([8168c89](https://github.com/shaug/atelier/commit/8168c8917d854917169f5b768fc15f7744d849b0))
- **work:** keep review-feedback changesets actionable and explicit
  ([f4445ab](https://github.com/shaug/atelier/commit/f4445ab6c9552f1e8fe9657499c15fe730225ed4))
- **work:** keep review-pending epics open and improve PR fallback content
  ([4e35679](https://github.com/shaug/atelier/commit/4e3567980e6df5866ebcf1b542601c71feef75b5))
- **work:** make reconciliation dependency-aware for merged changesets
  ([77ccddd](https://github.com/shaug/atelier/commit/77ccddd5862f71ea30f17bab84495f6c907b6ce7))
- **work:** persist review feedback cursor after feedback runs
  ([b04b2b5](https://github.com/shaug/atelier/commit/b04b2b53b1d97ff6b49f62807a727a4f6e776970))
- **work:** prefer live PR state for feedback candidate selection
  ([d512b2c](https://github.com/shaug/atelier/commit/d512b2c460853fcd9d357e8338daef383f08015c))
- **work:** prefer live PR state over stale review metadata
  ([b1edb53](https://github.com/shaug/atelier/commit/b1edb53c3ddc335c1195a966762961eb312ce82d))
- **work:** prefer oldest assigned epic
  ([532bce7](https://github.com/shaug/atelier/commit/532bce72e569de86c047aef9610b82c8bac6c703))
- **work:** prevent top-level PR gating deadlocks
  ([fb879fa](https://github.com/shaug/atelier/commit/fb879faa2a2efefcfff3b5ed4ef3d149d9e2629d))
- **work:** promote planned subtasks after parent completion
  ([64a97f6](https://github.com/shaug/atelier/commit/64a97f62b0804754a21cec15046aa10dd0475319))
- **work:** reclaim stale assignee on review feedback pickup
  ([6333398](https://github.com/shaug/atelier/commit/63333988b847183aac1e24e8a1a6786357ff7060))
- **work:** reclaim stale same-family worker claims
  ([d896de5](https://github.com/shaug/atelier/commit/d896de547ec8aa404691aad54773e4b561c1d3c6))
- **work:** recover premature cs:merged states into PR lifecycle
  ([e7c4f1e](https://github.com/shaug/atelier/commit/e7c4f1e9de1add79896e9e5af163f7558f39025e))
- **work:** recover when PR exists after create failure
  ([f8c3d4f](https://github.com/shaug/atelier/commit/f8c3d4f54311b48eaaca659f88567cde023df646))
- **work:** resume assigned epics before inbox
  ([52b6c09](https://github.com/shaug/atelier/commit/52b6c098ba4d4ce6f1a9c0645ace20bf4bae1a20))
- **work:** resume blocked changesets with publish signals
  ([8d391e9](https://github.com/shaug/atelier/commit/8d391e9310754c2e57465ee228c4ff83c58bb79b))
- **work:** retry blocked in-review changesets during feedback pickup
  ([0b3fd60](https://github.com/shaug/atelier/commit/0b3fd607b4795727b603f21fa873142d69b15019))
- **work:** run codex workers in non-interactive exec mode
  ([2046b03](https://github.com/shaug/atelier/commit/2046b03b60357c1c989c7ba6cf4b99b2466beb3b))
- **work:** run epic-as-changeset on the epic root branch
  ([155c616](https://github.com/shaug/atelier/commit/155c6165370140cb26d3533c80f186c22e082f7e))
- **work:** skip draft and assigned fallback ready-epics
  ([6c2f66c](https://github.com/shaug/atelier/commit/6c2f66c0ca5ab731830f58ac91e6df05ccda05ce))
- **work:** skip stalled epics and continue selecting work
  ([efc8683](https://github.com/shaug/atelier/commit/efc868346cb77d440d68ba73d2585971a3f54ebc))
- **work:** stabilize worker startup and ready-state flow
  ([8f9c4f2](https://github.com/shaug/atelier/commit/8f9c4f27f9470d88354fcbd3f7765bb28204d531))
- **work:** terminalize merged and closed PR changesets
  ([7e08338](https://github.com/shaug/atelier/commit/7e08338f5de17526e3569cfdc0307d4606043c23))
- **work:** tighten startup selection for draft and blocked feedback cases
  ([71b27b3](https://github.com/shaug/atelier/commit/71b27b3936352f608e6c3cd9137e2319475ef176))
- **work:** treat in-progress changesets as runnable
  ([445f33c](https://github.com/shaug/atelier/commit/445f33cba4b88f2d1aecc75712ef12c64eadf695))
- **worktrees:** create epic worktree from root branch
  ([c35e633](https://github.com/shaug/atelier/commit/c35e633c1cd39a16782e71f427903eb791f88462))
- **worktrees:** materialize root branch before changeset setup
  ([7125644](https://github.com/shaug/atelier/commit/7125644387c2dc9b2c850b6a312c530f615ae14e))
- **worktrees:** reconcile stale root-branch mappings
  ([7d25b5f](https://github.com/shaug/atelier/commit/7d25b5f5f8ca85d62e591b4b5d0bd6ece98b3476))
- **work:** use status-driven changeset readiness and normalize labels
  ([35d027b](https://github.com/shaug/atelier/commit/35d027b3a90bfede578c204d0da0a3bb7f4499ee))
- **work:** validate integrated sha claims against git graph
  ([857fa71](https://github.com/shaug/atelier/commit/857fa7100faf39128776a476e52f966d2bf40b7b))

### Performance Improvements

- **prs:** add bounded retries and runtime PR query caches
  ([a5be993](https://github.com/shaug/atelier/commit/a5be993796032d628b7a95248a6f18a7ca6f1339))

## [0.6.0](https://github.com/shaug/atelier/compare/v0.5.0...v0.6.0) (2026-01-26)

### Overview

This release expands day-to-day workspace operations while tightening migration
and upgrade behavior. Atelier adds `new`, `describe`, and improved remove/clean
flows, plus ticket-aware workspace defaults and identity/base-SHA tracking to
make branch intent clearer. It also introduces optional terminal adapters and
drops snapshot workflows, marking the transition toward more explicit,
branch-driven project state management.

### ⚠ BREAKING CHANGES

- remove atelier snapshot command and related tests.

### Features

- add atelier new command
  ([8a6c4ad](https://github.com/shaug/atelier/commit/8a6c4ad4b490ce7161beba250d3fd2392109fa85))
- add workspace snapshot command
  ([a8364d0](https://github.com/shaug/atelier/commit/a8364d048c565b78d9d0f937fe76463371746cb7))
- **ai:** add optional helpers for workspace open
  ([7e87f39](https://github.com/shaug/atelier/commit/7e87f3911dd4013956ea537a1bc65c9ab26714f7))
- **clean:** add dry-run preview
  ([62bcadd](https://github.com/shaug/atelier/commit/62bcaddd2ab0446fe40e0ab4f9490b153d80f03d))
- **cli:** add describe command
  ([808ac4d](https://github.com/shaug/atelier/commit/808ac4dbad63a261265b7789982229929a01d5ed)),
  closes [#46](https://github.com/shaug/atelier/issues/46)
- **cli:** add remove command and orphan cleanup
  ([c6f577a](https://github.com/shaug/atelier/commit/c6f577a52d6526ce982e751e6210a11549f8560b)),
  closes [#51](https://github.com/shaug/atelier/issues/51)
- **cli:** add workspace root option
  ([9b2de34](https://github.com/shaug/atelier/commit/9b2de342b767fafe9c20f89581e2b92957ef6cc2)),
  closes [#47](https://github.com/shaug/atelier/issues/47)
- **cli:** complete workspace names
  ([3fde23b](https://github.com/shaug/atelier/commit/3fde23b081470ac2921bf7a814c0ce605e69bf4a))
- **codex:** capture session id via pty
  ([75297bb](https://github.com/shaug/atelier/commit/75297bb0fc089d59e73b6514fd295fd05807fefd)),
  closes [#33](https://github.com/shaug/atelier/issues/33)
- **config:** add git path and provider metadata
  ([4cee306](https://github.com/shaug/atelier/commit/4cee30654360371a448ec84c1feca6be691eb946)),
  closes [#44](https://github.com/shaug/atelier/issues/44)
- **config:** add ticket support
  ([6b7e5f1](https://github.com/shaug/atelier/commit/6b7e5f1293df50027d8cdf990c06db9709918c36))
- **open:** add per-invocation yolo flag
  ([393b71a](https://github.com/shaug/atelier/commit/393b71a3e5cff1ed6143b85b99466c939c22772b))
- **open:** derive ticket workspace names
  ([3015e21](https://github.com/shaug/atelier/commit/3015e21c9881fdaced9a5f904bd299f2538e4862))
- **open:** enhance ticket workspace defaults
  ([336ef38](https://github.com/shaug/atelier/commit/336ef38d2dc9d6e4209bbe45f0c1f7242b7752ac))
- remove snapshot command
  ([5e8224b](https://github.com/shaug/atelier/commit/5e8224bdd96775c82dc432b378837d2f16e6e259))
- **templates:** align upgrade comparisons
  ([daa3aeb](https://github.com/shaug/atelier/commit/daa3aebc025acbde09c2afe0cc460bd709c304d9)),
  closes [#73](https://github.com/shaug/atelier/issues/73)
- **term:** add iTerm2 title adapter
  ([b807538](https://github.com/shaug/atelier/commit/b807538acd2fb0d2eed75f2c5b92d915870cc60b))
- **terminal:** add optional terminal adapter layer
  ([c3930da](https://github.com/shaug/atelier/commit/c3930da97f0badf70f171db9438a76d4a70573b5)),
  closes [#60](https://github.com/shaug/atelier/issues/60)
- **test:** add curated doctest collection
  ([1f78691](https://github.com/shaug/atelier/commit/1f786916dcd945a1ce31f036e6f42ec7d4f601ff))
- **workspace:** add workspace identity helpers
  ([35153ca](https://github.com/shaug/atelier/commit/35153cadd278d9c8985a27425f5e0144c02d8699)),
  closes [#50](https://github.com/shaug/atelier/issues/50)
- **workspaces:** remove legacy WORKSPACE.md support
  ([9532515](https://github.com/shaug/atelier/commit/953251534bace38581acc4d4cbf390cc707e40ae)),
  closes [#34](https://github.com/shaug/atelier/issues/34)
  [#35](https://github.com/shaug/atelier/issues/35)
- **workspace:** track base sha for committed work
  ([11d0358](https://github.com/shaug/atelier/commit/11d03589d71db266e8173a6dc40ca3ea2eb5f82c))

### Bug Fixes

- **clean:** guard remote branch deletion
  ([5897248](https://github.com/shaug/atelier/commit/5897248ac7fc47dacd8c7fb5cc1e6563ab666d0e))
- **cli:** fallback to branches when no workspaces
  ([7d56258](https://github.com/shaug/atelier/commit/7d562589b31ed17b2f391d9880ce74509d96223e))
- **cli:** handle shell completion env
  ([b779970](https://github.com/shaug/atelier/commit/b77997001b4566ad6e02cd56e3710f7c9c7b91f2))
- **cli:** show completion options
  ([818e6de](https://github.com/shaug/atelier/commit/818e6de3df438700f9004968194c335cdf7f84d6))
- **codex:** sync pty window size
  ([6d33456](https://github.com/shaug/atelier/commit/6d33456c336d9313d1b311fce95de1c2709c73d9))
- **exec:** normalize editor command inputs
  ([bd6107a](https://github.com/shaug/atelier/commit/bd6107ad18d59b69be6bfd1d96b7f8f33380c4c6)),
  closes [#59](https://github.com/shaug/atelier/issues/59)
- isolate codex sessions per workspace uid
  ([82ff375](https://github.com/shaug/atelier/commit/82ff375b774f7f9f3ee9c7699fe7f780a4464521))
- **open:** clear finalization tags in both repos
  ([b759186](https://github.com/shaug/atelier/commit/b75918694b02a68bf973279fbcaa4b5631cad26f)),
  closes [#70](https://github.com/shaug/atelier/issues/70)
- **open:** open success doc for new workspaces
  ([9e26f52](https://github.com/shaug/atelier/commit/9e26f524da0308e5ca3f3906599f34fc5b3d09a3)),
  closes [#38](https://github.com/shaug/atelier/issues/38)
- **open:** prefer exact branch matches
  ([89091e2](https://github.com/shaug/atelier/commit/89091e258423fdd958eb6fd82e8dd4e58266db3b)),
  closes [#58](https://github.com/shaug/atelier/issues/58)
- prefer exact branch matches in open
  ([00f7f4b](https://github.com/shaug/atelier/commit/00f7f4b3453dfa9208a028a96a90c8ddc24895b5))
- **sessions:** require exact workspace token
  ([503e60a](https://github.com/shaug/atelier/commit/503e60a5862ef57ce6d42b32644fac2d749bad31)),
  closes [#53](https://github.com/shaug/atelier/issues/53)
- **sessions:** scan jsonl for session_meta
  ([bf2a29a](https://github.com/shaug/atelier/commit/bf2a29a9940c58010cdabdd86a9f62c301cb2264)),
  closes [#71](https://github.com/shaug/atelier/issues/71)
- **shell:** normalize --shell override
  ([9fe023d](https://github.com/shaug/atelier/commit/9fe023de2fd8685873c31d4dc0aea87d508d9288))
- **template:** honor installed cache for template rendering
  ([1a1f3b3](https://github.com/shaug/atelier/commit/1a1f3b33708379389af7de049999da4455718925)),
  closes [#40](https://github.com/shaug/atelier/issues/40)
- **upgrade:** handle orphaned workspace configs
  ([d5ba536](https://github.com/shaug/atelier/commit/d5ba536f9d1e3c66988a28e856bb3dcd6a1d41c8)),
  closes [#57](https://github.com/shaug/atelier/issues/57)
- **upgrade:** migrate tickets in legacy config
  ([d29c30e](https://github.com/shaug/atelier/commit/d29c30ee38b00a3625b73b10b31cd67dc5e56b92))
- **upgrade:** prompt on modified files
  ([f89e04b](https://github.com/shaug/atelier/commit/f89e04bfcec17accbc5d31767ccfb27efe33c781))
- **upgrade:** prompt to remove legacy AGENTS
  ([2d5183a](https://github.com/shaug/atelier/commit/2d5183a23cffd7b24a68ab9ffd7582e0fde6e4d5))
- **wezterm:** set pane title via OSC
  ([c6d67eb](https://github.com/shaug/atelier/commit/c6d67eb794f90665f752102fd489987058d47928))

### Performance Improvements

- **cli:** speed workspace completions
  ([3e2ef97](https://github.com/shaug/atelier/commit/3e2ef97ccf3c2ea999f8796170a38fe3ab0b5f04))

### Miscellaneous Chores

- release 0.6.0
  ([d311b77](https://github.com/shaug/atelier/commit/d311b7798ddd9e27962b77fed22e08f619e18154))

## [0.5.0](https://github.com/shaug/atelier/compare/v0.4.0...v0.5.0) (2026-01-21)

### Overview

This release rounds out the workspace lifecycle and agent integrations: new
SUCCESS.md contracts, background/persist files, and clean finalization tagging
make workspaces more self-describing and easier to manage. The CLI and config
system level up with new config/template/edit and upgrade commands, a split
sys/user configuration model backed by pydantic, and workspace shell/exec plus
editor role support. On the agent side, resume support expands (Claude, Gemini
CLI, Copilot), and a handful of fixes smooth out config recovery, formatting,
and edge-case CLI behavior.

### Features

- add claude resume support
  ([8c6b54f](https://github.com/shaug/atelier/commit/8c6b54f22a5792c1bdc6772b68164c3d45800352))
- add config/template/edit commands
  ([0190934](https://github.com/shaug/atelier/commit/019093491a4edc77afe5fa78288c81df6ca89375)),
  closes [#22](https://github.com/shaug/atelier/issues/22)
- add persist/background workspace files
  ([e9e3160](https://github.com/shaug/atelier/commit/e9e3160a5ad3ab56cc9bc90b05a236b6a5882a04))
- add pydantic config models
  ([cb0438d](https://github.com/shaug/atelier/commit/cb0438dea03ef380a7d464d296f798ea2f94fbb6))
- add SUCCESS.md workspace contract
  ([b4f4955](https://github.com/shaug/atelier/commit/b4f49555edca66307441bef66ffc7bfe4c8b172b))
- add template upgrade policy
  ([e71de1e](https://github.com/shaug/atelier/commit/e71de1e01a8364532a6424eff11a63a126e58188))
- add upgrade command and template cache
  ([cc02362](https://github.com/shaug/atelier/commit/cc02362dfe9eab92ce068e3d5c55230f9dd35f7e))
- **agent:** add aider support
  ([519cc36](https://github.com/shaug/atelier/commit/519cc36448eb94bfb00deaa7c5508bf4aaa3efb6))
- **agent:** add gemini cli resume
  ([ec37503](https://github.com/shaug/atelier/commit/ec37503c6250b1c504a7b0e19c95aac7eed878f0)),
  closes [#26](https://github.com/shaug/atelier/issues/26)
- **agents:** finalize copilot support
  ([93257a3](https://github.com/shaug/atelier/commit/93257a3772f7a8b9c0eca6c7546eea859380a815)),
  closes [#27](https://github.com/shaug/atelier/issues/27)
- **cli:** add editor roles and work command
  ([23d7f52](https://github.com/shaug/atelier/commit/23d7f5221b242ead538c42fc797d495c68edd929)),
  closes [#29](https://github.com/shaug/atelier/issues/29)
- **cli:** add workspace shell and exec commands
  ([88e23c3](https://github.com/shaug/atelier/commit/88e23c3c0839d535df1c926bce72c21ffb0a4f3c)),
  closes [#30](https://github.com/shaug/atelier/issues/30)
- **config:** split sys/user config files (Fixed
  [#23](https://github.com/shaug/atelier/issues/23))
  ([340d7a6](https://github.com/shaug/atelier/commit/340d7a6d0e8eb6d6ff7818baf80f16779385ecb9))
- configure agent launching
  ([45a1296](https://github.com/shaug/atelier/commit/45a1296b91bc80ea96266df04b12b0623bbe361e)),
  closes [#24](https://github.com/shaug/atelier/issues/24)
- consolidate workspace policy files
  ([25141d6](https://github.com/shaug/atelier/commit/25141d6ee12b9bd0ef31009526b9f9a18ee0074a))
- gate clean on finalization tag
  ([423c385](https://github.com/shaug/atelier/commit/423c385091732d2ffff7d08274fdf83dcbd02ab5))
- identify projects by enlistment path
  ([06c880c](https://github.com/shaug/atelier/commit/06c880c0381345bd47db34d143809a25c92b9924))
- migrate cli to typer
  ([7b14260](https://github.com/shaug/atelier/commit/7b14260250f8bc579465818997ed8de6d995d5d9))
- shift workspace intent to WORKSPACE.md
  ([81f9dc3](https://github.com/shaug/atelier/commit/81f9dc344a795043cf72f6bc9ad6274945837649))
- use select prompts for config choices
  ([40a478b](https://github.com/shaug/atelier/commit/40a478bcb2f7d1f25fbe1192368afba9489d32f8))

### Bug Fixes

- **config:** recover from legacy and missing workspace configs
  ([a103969](https://github.com/shaug/atelier/commit/a1039694d04fc404bc14dd570348e20ab694990b))
- do not reapply branch prefix when already present
  ([65f79d6](https://github.com/shaug/atelier/commit/65f79d67a03f5e9d9e894fbd392e1e3542f67e75))
- fix the code and md formatting
  ([ea156fb](https://github.com/shaug/atelier/commit/ea156fb7d0eb520f85d5ebda823487e10a06c41e))
- match codex session prompt from event_msg
  ([5477f76](https://github.com/shaug/atelier/commit/5477f76e03c74f21fa4bb81cd81c9e6bf4c9e808))
- open workspace AGENTS.md in editor
  ([ac570a6](https://github.com/shaug/atelier/commit/ac570a617c443f00e139f8e96b5a6db02f8bcdf1))
- refresh tool install deps
  ([0e177e3](https://github.com/shaug/atelier/commit/0e177e3b75ed35cc06abe88c1e50b88315bc9d55))

## [0.4.0](https://github.com/shaug/atelier/compare/v0.3.0...v0.4.0) (2026-01-17)

### Overview

This release is a focused simplification pass. Default-branch selection moves to
runtime derivation and Atelier state/default handling is reduced, making project
setup and command behavior less config-heavy while preserving the new policy and
workflow scaffolding introduced in 0.3.x.

### Features

- derive default branch at runtime
  ([0140a4f](https://github.com/shaug/atelier/commit/0140a4ffecd290b922e0e88ccde10c81bd80c5fe))
- simplify atelier state and defaults
  ([6290250](https://github.com/shaug/atelier/commit/62902501eeb30b6239a7a8d705a86504ff61c0d6))

## [0.3.0](https://github.com/shaug/atelier/compare/v0.2.0...v0.3.0) (2026-01-15)

### Overview

This release establishes the first practical policy-driven workflow layer.
Atelier adds branch-policy controls in `open`, list/clean/status quality-of-life
commands, init prompt flags, and release/versioning guardrails, then follows
through with fixes around origin checks, workspace policy rendering, and
open-time behavior when repositories are missing or inputs are ambiguous.

### Features

- add branch policy overrides to open
  ([e596783](https://github.com/shaug/atelier/commit/e5967831d52044d39758b18d62feecf48dcda410))
- add cleanup and status options
  ([90d98bc](https://github.com/shaug/atelier/commit/90d98bcd974a157b99de821e56481c68bd0f9df8))
- add policy overlay stubs
  ([dc03b82](https://github.com/shaug/atelier/commit/dc03b8239a96aca0c22456e738bcf929342d491e))
- add policy overlay templates
  ([5314958](https://github.com/shaug/atelier/commit/53149586cf6fefe49668ae3ce05d913494ec495c))
- add versioning process
  ([6c00604](https://github.com/shaug/atelier/commit/6c0060429bd8c08d7bd5d5e71870d3e5eb160bfb))
- append branch summary on workspace open
  ([597273b](https://github.com/shaug/atelier/commit/597273bf33a335651d22867bbdafe29e53624059))
- **cli:** add list and clean workspace commands
  ([9b6f7a1](https://github.com/shaug/atelier/commit/9b6f7a14d29c06fb0eaa1fe6242f8c9df84746c1))
- **init:** add flags for prompt values
  ([da251ae](https://github.com/shaug/atelier/commit/da251aedaeb143cccf03eef99b1d492fff61a893))
- normalize workspace names
  ([28e98ad](https://github.com/shaug/atelier/commit/28e98adfa71c4c16c4b75f47b9dfc4e7ec86c0ad))
- **open:** add branch override flag
  ([bc18e11](https://github.com/shaug/atelier/commit/bc18e11fd8fbe3b128a9c4993b72a7f4ef745470))
- parallelize workspace status collection
  ([2a5c123](https://github.com/shaug/atelier/commit/2a5c123319c05ea32355911154b9949350c33055))
- prompt branch settings on init
  ([fbd932e](https://github.com/shaug/atelier/commit/fbd932ecfbef98a2d641d1d1f4846699acbfa84b))

### Bug Fixes

- correct workspace policy indentation
  ([76a4b83](https://github.com/shaug/atelier/commit/76a4b83c009a3d575298db1f2ed5834322711bee))
- enforce origin check and use workspace settings
  ([513c24b](https://github.com/shaug/atelier/commit/513c24ba45606c453e3dfaaf09f9d86649b0272f))
- open AGENTS before clone when repo missing
  ([8489fff](https://github.com/shaug/atelier/commit/8489fff6cfc28be70934d51c68282789a0a48626))
- strip workspace root from inputs
  ([396fa38](https://github.com/shaug/atelier/commit/396fa38a3eae61173a04437fa2f0fca1543c2f0a))

## [0.2.0]

- Initial release.
