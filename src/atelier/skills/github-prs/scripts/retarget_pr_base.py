#!/usr/bin/env python3
"""Retarget the base branch of a GitHub PR using gh CLI."""
...
def main(argv: Iterable[str]) -> int:
    parser = argparse.ArgumentParser(description="Retarget a GitHub PR base branch.")
    ...
    parser.add_argument("--base", required=True, help="New base branch")
    args = parser.parse_args(list(argv))

    try:
        number = find_pr_number(args.repo, args.head)
        if number is None:
            raise RuntimeError("No open PR found for the head branch")
        payload = read_pr(args.repo, number)
        current_base = payload.get("baseRefName")
        if current_base != args.base:
            run(
                [
                    "gh",
                    "pr",
