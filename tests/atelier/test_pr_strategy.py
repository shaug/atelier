...
def test_pr_creation_decision_blocks_on_open_parent() -> None:
    decision = pr_strategy.pr_creation_decision(parent_state="pr-open")
    assert decision.allow_pr is False
    assert decision.reason == "blocked:pr-open"


def test_pr_creation_decision_blocks_non_integrated_parent_states() -> None:
    decision = pr_strategy.pr_creation_decision(parent_state="pushed")
    assert decision.allow_pr is False
    assert decision.reason == "blocked:pushed"

    approved = pr_strategy.pr_creation_decision(parent_state="approved")
    assert approved.allow_pr is False
    assert approved.reason == "blocked:approved"


def test_pr_creation_decision_allows_when_parent_is_integrated() -> None:
    decision = pr_strategy.pr_creation_decision(parent_state="merged")
    assert decision.allow_pr is True
    assert decision.reason == "parent:merged"

    closed = pr_strategy.pr_creation_decision(parent_state="closed")
...
def test_pr_creation_decision_applies_sequential_policy(
    parent_state: str,
    allow_pr: bool,
    reason: str,
) -> None:
    decision = pr_strategy.pr_creation_decision(parent_state=parent_state)
