...
def test_integrate_changeset_updates_root_with_cas(tmp_path: Path) -> None:
    ...
    def fake_rev_parse(
        repo: Path, ref: str, *, git_path: str | None = None
    ) -> str | None:
        if ref == "main":
            return "oldsha"
        if ref == "work-branch":
    ...
    with (
        ...
        patch(
            "atelier.integration.exec_util.try_run_command", side_effect=fake_try_run
        ),
        patch("atelier.integration.git.git_rev_parse", side_effect=fake_rev_parse),
        patch(
            "atelier.integration.beads.update_changeset_integrated_sha"
        ) as update_sha,
    ):
    ...
    update_sha.assert_called_once_with(
        "epic.1", "newsha", beads_root=Path("/beads"), cwd=repo_root
    )
