...
def test_recovery_uses_diagnostic_payload_when_initial_lookup_misses(monkeypatch) -> None:
    issue = {"description": "changeset.work_branch: feature-branch\n"}
    mark_calls: list[str] = []
    update_calls: list[str] = []
    ...
    result = recovery.recover_premature_merged_changeset(
        ...
        mark_changeset_in_progress=lambda *_args, **_kwargs: mark_calls.append("in_progress"),
        update_changeset_review_from_pr=lambda *_args, **_kwargs: update_calls.append("updated"),
        handle_pushed_without_pr=lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("PR gate should not run when diagnostic lookup returns a PR payload")
        ),
    )
...
def test_recovery_merged_pr_requires_integration_proof() -> None:
    issue = {"description": "changeset.work_branch: feature-branch\n"}
    finalized: list[str] = []
    ...
    result = recovery.recover_premature_merged_changeset(
        ...
        lookup_pr_payload_diagnostic=lambda *_args, **_kwargs: (None, None),
        changeset_integration_signal=lambda *_args, **_kwargs: (False, None),
        finalize_terminal_changeset=lambda **_kwargs: (
            finalized.append("called") or FinalizeResult(continue_running=False, reason="terminal")
        ),
        mark_changeset_in_progress=lambda *_args, **_kwargs: None,
        update_changeset_review_from_pr=lambda *_args, **_kwargs: None,
        handle_pushed_without_pr=lambda **_kwargs: FinalizeResult(
