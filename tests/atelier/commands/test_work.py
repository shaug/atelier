...
def test_worker_opening_prompt_includes_review_feedback_context() -> None:
    ...
    assert "do not mention internal bead IDs" in opening_prompt
...
def test_render_changeset_pr_body_uses_scope_and_acceptance_criteria() -> None:
    issue = {
        ...
        "acceptance_criteria": (
            ...
            "- Admin synthetic memberships are paged after merge/sort."
        ),
    }

    body = work_cmd._render_changeset_pr_body(issue)
    ...
    assert "Automated PR for changeset" not in body
    assert "Generated by Atelier worker finalize fallback." not in body
...
def test_run_worker_once_persists_feedback_cursor_for_review_feedback() -> None:
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "atelier.commands.work._capture_review_feedback_snapshot",
                side_effect=[
                    work_cmd.ReviewFeedbackSnapshot(
                        feedback_at="2026-02-20T03:10:00Z",
                        unresolved_threads=2,
                        branch_head="abc1234",
                    ),
                    work_cmd.ReviewFeedbackSnapshot(
                        feedback_at="2026-02-20T03:11:00Z",
...
def test_review_feedback_progressed_when_unresolved_threads_drop() -> None:
...
def test_review_feedback_progressed_when_feedback_timestamp_advances() -> None:
    before = work_cmd.ReviewFeedbackSnapshot(
        feedback_at="2026-02-20T03:10:00Z",
        unresolved_threads=2,
        branch_head="abc1234",
    ...
    after = work_cmd.ReviewFeedbackSnapshot(
        feedback_at="2026-02-20T03:12:00Z",
        unresolved_threads=2,
        branch_head="abc1234",
    )
    assert work_cmd._review_feedback_progressed(before, after) is True
...
def test_run_worker_once_blocks_when_review_feedback_makes_no_progress() -> None:
    ...
    with ExitStack() as stack:
        ...
        mark_in_progress = stack.enter_context(
    ...
    assert summary.reason == "changeset_feedback_not_addressed"
    assert mark_in_progress.call_count >= 1
    mark_in_progress.assert_any_call(
        "atelier-epic.1",
        beads_root=Path("/beads"),
        repo_root=Path("/repo"),
    )
...
def test_finalize_reopens_closed_changeset_when_pr_is_open() -> None:
    with (
        patch(
            "atelier.commands.work.beads.run_bd_json",
            return_value=[
    ...
    mark_in_progress.assert_called_once_with(
        "atelier-epic.1", beads_root=Path("/beads"), repo_root=Path("/repo")
    )
    update_review.assert_called_once()
...
def test_finalize_child_pr_creation_respects_strategy_matrix(
    ...
    if expects_create:
        create_pr.assert_called_once()
        mark_in_progress.assert_called()
    else:
        create_pr.assert_not_called()
        assert expected_reason == "changeset_review_pending"
        mark_in_progress.assert_called_once()
