...
def test_planner_overview_reports_empty_sections(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("ATELIER_AGENT_ID", "atelier/planner/planner")
    emitted: list[str] = []
    ...
    with (
        ...
        patch("atelier.commands.plan.beads.list_inbox_messages", return_value=[]),
        patch("atelier.commands.plan.beads.list_queue_messages", return_value=[]),
        patch("atelier.commands.plan.planner_overview.list_epics", return_value=[]),
        patch("atelier.commands.plan.say", side_effect=emitted.append),
    ):
        plan_cmd.run_planner_overview(SimpleNamespace(agent_id=None))
...
def test_planner_overview_lists_claim_state_and_sorts_messages(tmp_path: Path) -> None:
    emitted: list[str] = []
    inbox = [
    ...
    with (
        ...
        patch("atelier.commands.plan.config.resolve_project_data_dir", return_value=tmp_path),
        patch("atelier.commands.plan.config.resolve_beads_root", return_value=Path("/beads")),
        patch("atelier.commands.plan.beads.list_inbox_messages", return_value=inbox),
        patch("atelier.commands.plan.beads.list_queue_messages", return_value=queued) as list_queue,
        patch("atelier.commands.plan.planner_overview.list_epics", return_value=[{"id": "at-1"}]),
        patch(
            "atelier.commands.plan.planner_overview.render_epics",
            return_value="Epics by state:\nOpen epics:\n- at-1 [open] Example",
        ),
        patch("atelier.commands.plan.say", side_effect=emitted.append),
    ):
        plan_cmd.run_planner_overview(SimpleNamespace(agent_id="atelier/planner/custom"))
