...
def test_create_message_bead_normalizes_contract_metadata_before_render() -> None:
    ...
    with (
        ...
        patch("atelier.beads._create_issue_with_body", return_value="atelier-56"),
        patch(
            "atelier.beads.run_bd_json",
            return_value=[{"id": "atelier-56", "title": "Hello"}],
        ),
    ):
        beads.create_message_bead(
            subject="Hello",
            body="Hi",
            metadata={"from": "alice", "thread": "at-epic.1", "msg_type": "notification"},
...
def test_repair_issue_event_history_overflow_compacts_notes_and_restores_mutability() -> None:
    ...
    def fake_direct_dolt_sql_command(*, query: str, beads_root: Path) -> exec_util.CommandResult:
        del beads_root
        sql_calls.append(["dolt", "sql", "-q", query])
    ...
    def fake_direct_dolt_sql_json(*, query: str, beads_root: Path) -> list[dict[str, object]]:
        del query, beads_root
        return [{"notes": issue_state["notes"]}]
    ...
    with (
        ...
        patch(
            "atelier.beads._run_direct_dolt_sql_command",
            side_effect=fake_direct_dolt_sql_command,
        ),
        patch(
            "atelier.beads._run_direct_dolt_sql_json",
            side_effect=fake_direct_dolt_sql_json,
        ),
    ):
    ...
    assert sql_calls
    assert sql_calls[0][:3] == ["dolt", "sql", "-q"]
...
def test_repair_issue_event_history_overflow_accepts_backend_readback_when_show_is_stale() -> None:
    initial_notes = "old note line\n" * 5000
    issue_state = {
        "id": "at-overflow",
        "title": "Overflowed issue",
...
def test_repair_issue_event_history_overflow_fails_closed_for_backend_readback_mismatch() -> None:
    ...
    def fake_direct_dolt_sql_command(*, query: str, beads_root: Path) -> exec_util.CommandResult:
        del beads_root
        assert query.startswith("UPDATE issues")
        return exec_util.CommandResult(
            argv=("dolt", "sql", "-q", query),
            returncode=0,
            stdout="",
